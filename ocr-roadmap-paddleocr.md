...
1. **上限重估**：`max_tokens` 只是保护性上限，放大不会让短输出变贵（模型自然 EOS 就停）。改为
   `est = clamp(total_chars × 2, 9000, 50000)`（通用护栏式上限：输出 Markdown/LaTeX 会相对输入膨胀，且 token 数不会超过字符数；护栏调大不影响自然收尾成本）。
...
> - `light._estimate_reconstruct_tokens` → `max(9000, min(50000, 输入字符×2))`（护栏式上限，无语料依据）；
