...
**图片 OCR 的行为（通用）****P4 起的两条入库/补缺口径**（影响 `library` 与 `catalog`）：
- 入库标题识别只认**带标点/带点编号/章节式**的标题（`1.` / `1、` / `1.2` / `（1）` / `第3章`），
  "数字 + 空格"（NFKC 之后的 `③ …`）不再当标题；纯文本笔记可用 `HEADING_NUM_LOOSE=1` 回退；
- 只要有骨架，**输出侧补缺不再参与**（覆盖由骨架侧保证），且程序不再写入
  `核心知识点` 这类占位名。

：多张图片**并发识别**，结果**按 `docs` 顺序拼接**（顺序可预期，
