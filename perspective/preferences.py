...
PERSONAL_VIEW_DIRECTIVE = (
    ...
    "（组名行独占一行、不加 `- `；块内可按数字序号 `1. ` 或 `- ` 一条一行；没有内容的块不出现）：\n"
    f"  {SELF_GROUP_ROW}\n"
    "  1. （本人条目，不写自己的姓名）\n"
    "  **他人姓名**：\n"
    "  1. （该人的条目，不重复姓名）\n"
    ...
    "**他人的按其姓名分组**（姓名照原文），组内同样可按数字序号或 `- ` 一条一行；"
    ...
    "- **文风严谨去AI味**：严禁使用任何 emoji 表情符号（如 🔄、⏸、✅、⛔ 等）；正文不要逐条机械填充「进行中」或「未提及」，状态之外必须交代实质进展内容。\n"
...
PERSONAL_TEMPLATE_VIEW_DIRECTIVE = (
    ...
    "- **文风严谨去AI味**：严禁使用任何 emoji 表情符号（如 🔄、⏸、✅、⛔ 等）；严禁机械填充「进行中」「未提及」等无意义占位词，括号内的时限、交付物仅在原文明确提及时代入，未提及要素直接省略；他人事项必须统一归集在对应姓名下，严禁自造讨论长句作为组名独占行。\n"
    "- **分栏组名与序号**：行动项与风险卡点栏中，归属本人的条目排在最前并独占一行「**与我相关**：」，组内顶层事项优先按数字序号（1. 2. 3.）标出；他人分工按姓名分组「**姓名**：」；前置依赖列在「**前置依赖**：」下；没有内容的组不出现。\n"
