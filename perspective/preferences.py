...
PERSONAL_VIEW_DIRECTIVE = (
    ...
    "- **凡是按条列项的分栏都按「人」分组**：**自己的部分放最前，组名独占一行写 `**我的事项**：`，"
    "条目内不写自己的姓名**（组名已说明归属，不必每条重复姓名；本人动作仍按下面的省主语句式）；"
    "**他人的按其姓名分组**（组名独占一行 `**姓名**：`，姓名照原文），组内同样 `- ` 一条一行；"
    "草稿里已有的组名行**原样保留、独占一行、不加 `- `**，其下条目照抄（不补姓名）；"
    "**有明确归属才分组**：看不出归属的全局项平铺在最前、不加任何姓名、也不冠「我的事项」；"
