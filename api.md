...
- 模板的三种形式（占位符 / 格式规范 / 自然语言）与自动判型处理方式见 [README.md](README.md)「自定义输出模板」；开关 `TEMPLATE_ROUTER=off` 可退回旧行为
...
- `role_template`：挂已有职业模板（`assets/profiles/{名}.json`）；模板作底，`user.json` 里**写了且非 null 的键整段替换**（列表覆盖、不拼接），没写的键继承模板；找不到模板 → 400。
...
- **真人档的正文人名口径**：本人动作**省主语**（「今天先把配置改完，明天给结论」）、他人动作写真名（「王工要求周五前完成联调」）、只有待办/提醒/被点名才用「你」；分工、责任人、引语与第三方姓名一律真名，同一句与相邻两句不混用。客观/职业模板仍是第三人称。
