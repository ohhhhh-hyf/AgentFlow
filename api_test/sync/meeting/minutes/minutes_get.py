"""下载 minutes 产物：GET /api/agent/v1/file/{request_id}/{file_name}。

统一入口：域与任务名不再出现在路径里，下载只认 request_id + file_name
（产物目录 data/{user_id}/output/{request_id}/ 由 request_id 唯一确定）：
- request_id：POST /api/agent/v1（"domain":"meeting","task":"minutes"）响应里的 request_id
- file_name ：该响应里的 data.file_name（本脚本默认 minutes.html）
...
想在浏览器里直接看页面版（不落盘），用预览端点（唯一保留域/线名的形态）：
    GET /api/v1/meeting/minutes/preview?request_id=…&user_id=…
想取 Markdown 正文：把 FILE_NAME 换成 result.md（minutes 线的 md 产物名就是 result.md）。

用法：python minutes_get.py
"""
...
FILE_NAME = "minutes.html"       # ← 响应里的 data.file_name；Markdown 用 result.md

if not REQUEST_ID:
    print("请先把 REQUEST_ID 填成 POST minutes 响应里的 request_id")
    raise SystemExit(1)
