...
- 思路 3：连续性方程 $\frac{\partial P(x,t)}{\partial t} + \frac{\partial j(x,t)}{\partial x} = 0$，其中 $\rho = \psi^*\psi = |\psi|^2$，$j = -\frac{i\hbar}{2\mu}(\psi^*\nabla\psi - \psi\nabla\psi^*)$。连续 $\Rightarrow \psi, \psi'$ 连续
...
3. **$x > a$ 区域**：有方程 $\psi''(x) - \frac{2\mu}{\hbar^2}(V_0 - E)\psi(x) = 0$。令 $\beta = \sqrt{\frac{2\mu(V_0 - E)}{\hbar^2}}$，则 $\psi(x) = Be^{-\beta x} + Ce^{\beta x}$。
...
$$
\sin^2(ka) = \frac{2a\beta}{2a\beta + \sin(2ka)}, \quad \sin(2ka) = 2\sin(ka)\cos(ka)
$$
...
若级数无穷，则 $H(s) \sim c e^{s^2}$，导致 $\phi(s)$ 按 $e^{s^2/2}$ 量级增长，不可归一化。因此级数必须截断为有限项：存在 $j = n$ 使 $a_n \neq 0$，$a_{n+2} = 0$，即 $\lambda = 2n + 1$。
...
$$\frac{1}{\Phi}\frac{d^2\Phi}{d\phi^2}=-m^2 \Rightarrow \frac{d^2\Phi}{d\phi^2}+m^2\Phi(\phi)=0 \Rightarrow \Phi_m(\phi)=e^{im\phi}$$

对于勒让德方程：

$$\frac{1}{\sin\theta}\frac{d}{d\theta}\left(\sin\theta\frac{d\Theta}{d\theta}\right)+\left(\lambda-\frac{m^2}{\sin^2\theta}\right)\Theta(\theta)=0$$

为使 $\Theta(\theta)$ 在区间 $[0,\pi]$ 有限，$\lambda$ 只能取：
...
$$\int_0^\pi\int_0^{2\pi}|\Theta(\theta)\Phi(\phi)|^2\sin\theta\,d\theta\,d\phi=1$$
...
$$Y_{lm}(\theta,\phi)=N_{lm}\Theta_{lm}(\theta)\Phi_m(\phi)=N_{lm}P_l^{|m|}(\cos\theta)e^{im\phi}$$

其中 $P_l^{|m|}(\cos\theta)$ 为连带勒让德函数。

**正交归一关系：**

$$\int_0^\pi\int_0^{2\pi}Y_{lm}^*(\theta,\phi)Y_{l'm'}(\theta,\phi)\sin\theta\,d\theta\,d\phi=\delta_{ll'}\delta_{mm'}$$

球谐函数是 $L^2$ 和 $L_z$ 的**共同本征函数**。
...
$j_\varphi = \frac{e\hbar m}{2\mu r \sin\theta} |\psi|^2 = \frac{e\hbar m}{2\mu r \sin\theta} |R_{nl} P_l^m|^2$。
...
$$F_x=q\left[-\frac{\partial\phi}{\partial x}-\frac{\partial A_x}{\partial t}+\frac{\partial A_x}{\partial x}\dot{x}+\frac{\partial A_x}{\partial y}\dot{y}+\frac{\partial A_x}{\partial z}\dot{z}\right]=-q\frac{\partial\phi}{\partial x}-q\frac{dA_x}{dt}+q\frac{\partial}{\partial x}(\mathbf{v}\cdot\mathbf{A})$$
...
由拉格朗日方程 $\frac{d}{dt}\left(\frac{\partial L}{\partial \dot{x}}\right)-\frac{\partial L}{\partial x}=F_x=-q\frac{\partial}{\partial x}(\phi-\mathbf{A}\cdot\mathbf{v})$，其中：
...
**三维欧氏空间**，线元 $ds^2 = (dx)^2 + (dy)^2 + (dz)^2 = \sum_{i,j} g_{ij} dx^i dx^j$，$g_{ij} = \delta_{ij} = \begin{pmatrix} 1 & 0 & 0 \\ 0 & 1 & 0 \\ 0 & 0 & 1 \end{pmatrix}$
...
在曲线坐标 $(u, v, w)$ 中，考虑一个由坐标面围成的体积元 $dudvdw$。

**平行于 $d$ 的四个面**，在剩下两面中一个面的通量为：

两个面的通量之差为：

$$\left( \frac{\partial}{\partial u} (J a_u) \right) du \, dv \, dw = \frac{\partial}{\partial u} (J a_u) \, dudvdw$$

同理，另外两个通量差为：

$$\frac{\partial}{\partial v} (J a_v) \, dudvdw, \quad \frac{\partial}{\partial w} (J a_w) \, dudvdw$$

因此：

$$\nabla \cdot \mathbf{a} \, dV = \left( \frac{\partial}{\partial u} (J a_u) + \frac{\partial}{\partial v} (J a_v) + \frac{\partial}{\partial w} (J a_w) \right) dudvdw$$
...
其中 $J = H_u H_v H_w$ 为雅可比行列式。

**特例**：若 $(u, v, w) = (r, \theta, \phi)$（球坐标），则：
...
考虑 $\text{rot} \, \mathbf{a}$ 在 $u$ 轴上的投影，取 $\mathbf{n}$ 为正方向，$S$ 面是 $u = \text{常数}$ 的曲面，曲面 $S$ 中的曲线 $L$ 设为 $M_1 M_2 N_2 N_1$。

沿 $M_1 M_2$ 的线积分：

$$\int_{M_1 M_2} \mathbf{a} \cdot d\mathbf{l} = a(u, v, w) \cdot d\mathbf{l} = a(u, v, w) \cdot H_v \, dv = a_v (u, v, w) H_v (u, v, w) \, dv$$

其中 $M_1 = (u, v, w)$，$M_2 = (u, v + dv, w)$。
...
$$\oint \mathbf{a} \cdot d\mathbf{l} = \left[ \frac{\partial (a_w H_w)}{\partial v} - \frac{\partial (a_v H_v)}{\partial w} \right] dv \, dw = (\text{rot} \, \mathbf{a})_u \, dv \, dw$$
...
$\frac{}{2\mu}(x-y)+\frac{}{2\mu}(x^2+y^2)+V()$，$\rho^2=x^2+y^2=x-y$

$A_{1m}=E_{4nm}$，$\psi_{4nm}(r,\theta,\phi)=R_{nl}(r)Y_{lm}(\theta,\phi)$

则 $\left[\frac{}{2\mu}\left(r^2+\frac{}{r^2}\right)+\frac{}{2\mu r^2}\right]R(r)Y(\theta,\phi)=-\frac{}{}+\frac{}{}mR(r)Y$

于是有 $\frac{}{}\cdot\left(r^2\frac{}{r}\right)+\frac{}{r^2}\frac{}{}$

**注意** ① 左式 $=\frac{}{r^2}\left(r^2\frac{}{r}\right)-\frac{}{r^2}\left[\frac{1}{\sin\theta}\frac{}{\partial\theta}\left(\sin\theta\frac{}{\partial\theta}\right)+\frac{1}{\sin^2\theta}\frac{^2}{\partial\phi^2}\right](\theta,\phi)=E()(\theta,\phi)$

$-\frac{}{r^2}\left(r^2\frac{}{r}\right)Y(\theta,\phi)-\frac{}{r^2}\left[\frac{1}{\sin\theta}\frac{}{\partial\theta}\left(\sin\theta\frac{}{\partial\theta}\right)+\frac{1}{\sin^2\theta}\frac{^2}{\partial\phi^2}\right](\theta,\phi)=E()$

$\frac{}{r^2}\left(r^2\frac{}{r}\right)+\frac{}{r^2}\left[\frac{1}{\sin\theta}\frac{}{\partial\theta}\left(\sin\theta\frac{}{\partial\theta}\right)+\frac{1}{\sin^2\theta}\frac{^2}{\partial\phi^2}\right]=E$

令 $E_0=E-\omega_l m\hbar=E-m$，代 $\lambda V(r)=-k-\lambda k$

则 $\frac{}{r^2}\left(r^2\frac{}{r}\right)R(r)-\frac{}{r^2}\left(-\frac{}{}-\lambda k\right)-\left(\omega_l m-E\right)=0$

$\frac{}{}+\frac{}{}+\left(E_0+\frac{}{}\right)R(r)=0$，对比碱金属原子方程，有 $E_0=-\frac{}{}=-\frac{}{}$
...
① 可不计分子本身的大小（分子线度为 $d$，则 $d \ll \bar{l}$，$a \cdot P \cdot N_A \approx M \cdot d$，即可不计分子本身的大小）
...
$$v^2 = v_x^2 + v_y^2 + v_z^2 = \overline{v^2} = \frac{3kT}{m}$$
...
$$E = \frac{1}{2}m(r^2\dot{\theta}^2 + r^2\sin^2\theta\,\dot{\varphi}^2) = \frac{1}{2}m r^2(\dot{\theta}^2 + \sin^2\theta\,\dot{\varphi}^2)$$

对于双原子分子，两质点绕系统质心转动可约化为质量为 $\mu$ 的单体转动问题。

### ④ 转动动能与角动量

系统转动能量 $T = \frac{1}{2}m_1 r_1^2(\dot{\theta}^2 + \sin^2\theta\,\dot{\varphi}^2) + \frac{1}{2}m_2 r_2^2(\dot{\theta}^2 + \sin^2\theta\,\dot{\varphi}^2)$

即 $T = \frac{1}{2}\mu r^2(\dot{\theta}^2 + \sin^2\theta\,\dot{\varphi}^2) = \frac{1}{2}\mu r^2(\dot{\theta}^2 + \sin^2\theta\,\dot{\varphi}^2)$

$P_\theta = \mu r^2 \dot{\theta} = P_{\theta_1} + P_{\theta_2}$，$P_\varphi = \mu r^2 \sin^2\theta\,\dot{\varphi}$（与 $\theta$、$\varphi$ 共轭的动量）

$$T = \frac{1}{2}\mu r^2\dot{\theta}^2 + \frac{1}{2}\mu r^2\sin^2\theta\,\dot{\varphi}^2 = \frac{1}{2\mu r^2}\left(P_\theta^2 + \frac{P_\varphi^2}{\sin^2\theta}\right)$$
...
粒子的状态是连续的，粒子和系统的微观运动状态不可数，人为划出最小的格子。将 $p,q$ 分为等间隔区域，$\delta p_j \delta q_j = h_0$（$\delta q_1 \delta p_1 \cdots \delta q_r \delta p_r = h_0$），相格 $h_0$ 代表一个粒子态。子相宇空间小体元 $\Delta\mu_j$（$j=1,2,\cdots,l$）中粒子运动状态数为 $\Delta\omega_j = \dfrac{\Delta\mu_j}{h_0}$，$a_j$ 个粒子在 $\Delta\omega_j$ 个运动状态上分布的可能微观状态数为 $\dfrac{(\Delta\omega_j)^{a_j}}{a_j!}$，多个粒子可以处于同一相格内。
...
$$W=\frac{(2n)!}{n!n!}\frac{(n+\Delta n)!(n-\Delta n)!}{(2n)!}=\frac{(n!)^2}{(n+\Delta n)!(n-\Delta n)!}$$
...
第一象限 $0\sim$ 范围内的总量子态数 $\Omega(E)=\frac{1}{h^3}\times\frac{4\pi}{3}\left(2mE\right)^{3/2}abc=\frac{4\pi V}{3h^3}(2mE)^{3/2}$。

$2\mu E r_0 \sin^2\theta$，$P_\phi^2$。
...
- $A|\psi\rangle = |A\psi\rangle$，在右矢空间中，$|\psi\rangle \to |\psi'\rangle = |A\psi\rangle$，默认 $A$ 向右作用；在左矢空间中，$\langle \psi| \to \langle \psi'| = \langle \psi|A^\dagger$，算符 $A^\dagger$（$A$ 的转置共轭）向左作用。$\langle \psi|A^\dagger = \langle A\psi| = \langle \psi'|$。若 $A$ 为厄米算符，则 $\langle \psi'| = \langle \psi|A$。
...
$$[l_x, l_y] = [yp_z - zp_y, zp_x - xp_z] = [y, z]p_xp_z - [z, xp_z] + [z, xp_z]$$

$$= [y, zp_x] + [z, xp_y] = y[p_z, l_y] + x[-l_z, p_y] = -y(i\hbar x) - x(-i\hbar p_y) = i\hbar l_z$$
...
$\bar{l}_x^2 = \langle lm|l_x^2|lm\rangle = \langle lm|\frac{1}{2}(l_+ l_- + l_- l_+)|lm\rangle = \frac{1}{2}[l(l+1) - m^2]\hbar^2$

$\bar{l}_y^2 = \langle lm|l_y^2|lm\rangle = \langle lm|\frac{1}{2}(l_+ l_- + l_- l_+)|lm\rangle = \frac{1}{2}[l(l+1) - m^2]\hbar^2$
...
其中 $\langle[\hat{A},\hat{B}]\rangle = \langle \hat{A}\hat{B} - \hat{B}\hat{A} \rangle$ 对成立，$\Delta A = \overline{A - \bar{A}}$，$\Delta B = \overline{B - \bar{B}}$。也即：

$$[\hat{A},\hat{B}] = [\Delta A + \bar{A},\ \Delta B + \bar{B}] = (\Delta A + \bar{A})(\Delta B + \bar{B}) - (\Delta B + \bar{B})(\Delta A + \bar{A}) = [\Delta A, \Delta B]$$

$$\Delta A = \sqrt{\overline{A^2} - \bar{A}^2} = \sqrt{\langle(A - \bar{A})^2\rangle},\quad \Delta B = \sqrt{\overline{B^2} - \bar{B}^2}$$

要证：

$$\langle(A - \bar{A})^2\rangle \cdot \langle(B - \bar{B})^2\rangle \geq \frac{1}{4}|\langle[A - \bar{A},\ B - \bar{B}]\rangle|^2$$

令 $x = A - \bar{A}$，$y = B - \bar{B}$，要证变为：
...
$\because \forall \lambda$，$\langle\phi|\phi\rangle \geq 0$，$\therefore \langle[\hat{A},\hat{B}]\rangle^2 \leq 4\langle\hat{A}^2\rangle\langle\hat{B}^2\rangle$，即：

$$\langle\hat{A}^2\rangle\langle\hat{B}^2\rangle \geq \frac{1}{4}|\langle[\hat{A},\hat{B}]\rangle|^2$$
...
又 $\langle k | F | k \rangle = F$ 为实数（厄米算符平均值），且 $\langle k | k \rangle = 1$，
∴ $\lambda_k = F$ 为实数。
...
其中 $C_k$ 为用 $|k\rangle$ 将 $|\psi\rangle$ 做展开时的展开系数。

### 定理：哈密顿算符本征态的完备性

**定理**：哈密顿算符 $H$ 为厄米算符，满足本征方程 $H|k\rangle = E_k |k\rangle$。对体系的任一归一化态 $|\psi\rangle$，若 $H = \langle \psi | H | \psi \rangle$ 有下界（总大于某常数）但无上界，则 $H$ 的本征态 $|k\rangle$ 的集合构成体系的一个**完备集**，即体系的任一量子态 $|\psi\rangle$ 可用 $|k\rangle$ 来展开。
...
条件：$\frac{\partial\hat{A}}{\partial t}=0$ 且 $[\hat{H},\hat{A}]=0$，则 $\hat{A}$ 为守恒量。
...
设 $F=(A_1, A_2, \cdots, A_n)$ 是一组力学量完全集，$|k\rangle$ 是其共同本征态，其中 $k$ 表征所有量子数。$|k\rangle$ 是正交归一的，满足 $\langle k|m\rangle = \delta_{km}$（$k=m$ 时为 1，否则为 0），$|k\rangle$ 是完备的，即体系中任一量子态可按 $|k\rangle$ 展开。$\{|k\rangle\}$ 构成一个表象，可称为 $F$ 表象。$F$ 构成无穷维的希尔伯特空间，量子态是希尔伯特空间中的一个矢量，$|\psi\rangle = \sum_k a_k |k\rangle$，则 $a_k = \langle k|\psi\rangle$ 为内积，也可视为投影。

### 表象间的转化

$F$ 表象中，$|\psi\rangle = \sum_k a_k |k\rangle$，$\psi$ 在 $F$ 表象中可用系数列向量表示为 $a$。$a'$ 与 $a$ 的转化实际上是基矢之间的转化。
...
从矢量的角度考虑，$|\psi\rangle$ 是列向量 $e_k$ 的线性组合。要消去基矢，则基矢组之间通过 $S$ 矩阵相联系，合并系数列。

由正交归一性：
- $\langle k'|k\rangle = \delta_{k'k}$，$\langle k|k\rangle = 1$
- $|k\rangle = \sum_\beta S_{\beta k} |\beta\rangle$，$\langle k'|k\rangle = \left(\sum_\beta S_{\beta k'}^* \langle \beta|\right)\left(\sum_\beta S_{\beta k} |\beta\rangle\right) = \sum_\beta S_{\beta k'}^* S_{\beta k} = \delta_{k'k}$
- $\langle m|k\rangle = \left(\sum_\beta S_{\beta m}^* \langle \beta|\right)\left(\sum_\beta S_{\beta k} |\beta\rangle\right) = \sum_\beta S_{\beta m}^* S_{\beta k} = \delta_{mk} \Rightarrow S^\dagger S = SS^\dagger = I$

**$S$ 为幺正矩阵。**

基矢变换关系：
$$(\tilde{e}_1, \tilde{e}_2, \cdots, \tilde{e}_k, \cdots) = (e_1, e_2, \cdots, e_k, \cdots) S$$

其中矩阵元 $S_{ij} = \langle e_i, \tilde{e}_j \rangle$，即 $(\tilde{e}_1, \tilde{e}_2, \cdots) = (e_1, e_2, \cdots) S$。
