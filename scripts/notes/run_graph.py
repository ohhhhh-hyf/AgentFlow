...
接口：POST /api/v1/notes/graph
用法（先启动服务，agentflow 环境）：
    python scripts/notes/run_graph.py
"""
# ============================================================================
...
NOTES = """极限的定义：函数在某点的极限描述的是当自变量趋近该点时函数值的变化趋势。
连续性是极限存在且等于函数值。导数是变化率的极限。
幂级数解法通过构造递推的系数关系求解。
概率密度描述随机变量在某个取值附近的概率分布，径向概率分布关注到原点距离的分布。"""
...
def main() -> None:
    ...
    resp = requests.post(
        ...
        json={
            "domain": "notes",
            "task": "graph",
            "texts": {"transcript": NOTES},
            "docs": ["note.md"],    # 笔记 .txt/.md 文件（必填），需已放入 data/1/docs/
            "extra": {
                "template": "",  # 空=不套模板；可填 29 个值
                "profile": "",   # 空=客观全员
                "project": "",
                "subject": "",   # 学科名（可选，按学科合并图谱）
