"""library 接口基础测试骨架 —— 资料入库。

接口：POST /api/v1/notes/library
用法（先启动服务，agentflow 环境）：
    python scripts/notes/run_library.py
说明：images/files 是文件名，服务端从 data/{user_id}/images|files/ 取；
      多图 OCR 较慢（1~10 分钟），本骨架用 1 张图。
"""
# ============================================================================
...
USER = "1"  # X-User-Id 必填（知识库按用户隔离）
SUBJECT = "phy"  # 学科（可选，建议填以按学科分类）


def main() -> None:
    """调用接口，解析返回 data 并按 Markdown 展示。"""
    resp = requests.post(
        f"{BASE}/api/v1/notes/library",
        headers={"X-User-Id": USER, "X-Request-Id": REQUEST_ID},
        json={
            "domain": "notes",
            "task": "library",
            "texts": [],                                 # 可传补充文本，空即可
            "docs": ["U202314751_1.jpg","U202314751_2.jpg","U202314751_3.jpg","U202314751_4.jpg","U202314751_5.jpg"],   # 图片/文档，需已放入 data/1/docs/
            "extra": {
                "template": "",
                "profile": "",
                "project": "",
                "subject": SUBJECT,                      # 学科（建议填，按学科分类）
                "style": "",
            },
        },
        timeout=900,
