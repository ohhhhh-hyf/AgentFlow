"""catalog 接口基础测试骨架 —— 知识目录。

接口：POST /api/v1/notes/catalog
用法（先启动服务，agentflow 环境）：
    python scripts/notes/run_catalog.py
说明：基于该用户已入库知识库生成；extra.subject 必填；
...
def main() -> None:
    ...
    print("（后续 checklist 的 docs 传上面 data.file_name 的时间戳文件名，即可基于该目录生成复习清单）")
    print("--- Markdown 内容 ---")
    print(md_text)
