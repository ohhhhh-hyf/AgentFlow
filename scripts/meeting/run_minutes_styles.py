"""minutes_styles 接口基础测试骨架 —— 多样式纪要。

接口：POST /api/v1/meeting/minutes_styles
用法（先启动服务，agentflow 环境）：
    python scripts/meeting/run_minutes_styles.py
"""
# ============================================================================
...
# extra.style 可填：time / logic / causal / party / urgency
STYLE = "causal"
...
def main() -> None:
    ...
    resp = requests.post(
        f"{BASE}/api/v1/meeting/minutes_styles",
        ...
        json={
            ...
            "extra": {
                ...
                "project": "",    # 会议关联项目 ID
                "subject": "",
                "style": STYLE,   # time / logic / causal / party / urgency
            },
        },
        timeout=300,
    )
