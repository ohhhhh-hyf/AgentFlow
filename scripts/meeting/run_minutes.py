"""minutes 接口基础测试骨架 —— 会议纪要提取。

接口：POST /api/v1/meeting/minutes
用法（先启动服务，agentflow 环境）：
    python scripts/meeting/run_minutes.py
"""
# ============================================================================
...
def main() -> None:
    ...
    resp = requests.post(
        ...
        json={
            ...
            "docs": [],
            "extra": {
                "template": "",   # 空=不套模板；可填 29 个值（如 meeting_minutes_team_meeting）
                "profile": "tester",    # 空=客观全员；可填 developer 等 7 个值
                "project": "",    # 会议关联项目 ID（开项目记忆）
                "subject": "",    # 学科名（library/catalog/checklist 用）
                "style": "",      # 仅 minutes_styles 用（time/logic/causal/party/urgency）
            },
        },
        timeout=300,
    )
