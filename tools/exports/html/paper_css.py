"""LaTeX Paper 风格的内联样式表（各线 HTML 共用）。

2026-09-22 从 ``domain/meeting/memory/render.py`` 提到这里：它是**纯样式**，被
meeting 域的三类 HTML 与 tools 侧的 ``exports/html/consensus_decision.py`` 共用。
留在域里会让 tools 的渲染器反向依赖域包，故独立成模块。
"""
from __future__ import annotations


def latex_paper_css() -> str:
    return """
    body {
      margin: 0;
      padding: 14px 12px;
