...
标题层级铁律（**级别表达知识层级，不表达版面位置**；违反会直接毁掉知识目录的章/节结构）：
- 标题出现在页面顶部、居中、字号大，都**不构成**升级别的理由：`heading_level_hint` 是"本页相对"的版面提示，只作参考
- 判级别看**内容的外延**，不看标题的长短与位置：
  一级（#）= 一个独立的知识范围（一个领域、一门课的一个大单元、一个被反复展开的主题；
    它的跨度通常覆盖多页、能容纳多个二级标题）；
  二级（##）= 该范围内的一个自成一段的侧面/小节（只讲这个范围的一个方面，通常不跨太多页）；
  三级（###）= 二级之下更细的知识点、子节
- **同一逻辑层必须同级别**：同一个名字（含「X（续）」）在别处是什么级别，这里就是什么级别；
  「（续）」标题与它续的那一节同级别
- 一章通常跨多页：本页内容若仍在上一页那一层的范围内（同一知识范围），标题用二级；
  **同一页里出现多个一级标题，多半是把"侧面"误当成了"范围"**——先判断它们是否只是同一范围的不同侧面
- 一级标题全篇应当只有少数几个（通常 ≤ 10 个），且明显少于二级标题；二级又少于三级
...
def reconstruct_markdown(lines: list[dict], *, max_tokens: int = 5000, context: str = "") -> str:
    ...
    if context:
        user_prompt = (
            "【跨页层级锚点】上一页的标题结构（版面推断的相对层级，供对齐用）：\n"
            + context.strip()[:240]
            + "\n对齐要求：**同一逻辑层用同一级别**——上一页给出的层级就是本页的参照；"
            "本页若仍在同一知识范围内，标题用二级，不要因为出现在页面顶部就用一级。"
            "只有本页内容确为一个新的独立知识范围时才另起一级。\n\n"
            + user_prompt
        )
    try:
        import asyncio
