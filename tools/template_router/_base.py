    ...
    if seg.get("heading"):
        ...
        if _HAS_SIZE_SPEC_RE.search(hint):
            ...
            desc += (
                ...
                "「以分享为主，未形成明确结论」）④ 1–3 个关键数字（原文没有的不编）；"
...
- **未声明全文字数上限时，篇幅以原文为参照**：与原文同量级（90%–110%）即可，不必强压；按栏目把原文事实写清——宁可接近原文长度，也不丢原文已有的事实。**这一条只管整篇总量，不管单栏单段**：单栏/单段上限见形态规则与字段清单，内容多靠增加段数与分条承载，不是把一段写长
- **冲突裁决**：**保真优先**（不删事实、不改口径与人名数字）→ 其次保结构（栏目齐全）→ 最后才压表达；篇幅以原文量级为参照，明显超量时先删套话与同义反复、再归并同类——**压缩不等于删事实**：数字、日期、人名、决策、承诺、模块进展与风险一律保留
- 「一段话概括 / 概况」= **最多 3 段、每段不超过 400 字**；**总述栏（首栏）只写一段、不超过 400 字**；只写该栏主题的概括（背景、目的、结论口径），可带 1–2 个关键数字作锚点；人物/数据清单、事项进展、承诺与待办等明细**归各自栏目，本栏不复述**；信息少时一两句即合格，**禁止只有一句空泛总结**
