...
def _describe_field(index: int, seg: dict) -> str:
    ...
    elif re.search(r"概括|概况|综述|摘要", lead):
        desc += "：写成完整段落（信息充足时 4–10 句为上限、信息少时可短），写清该栏主题的事实与要点，带原文数字/地点/责任人；信息多时可分点补充；禁止单句空话"
...
## 输出约定
1. fields 的 key 为字符串数字，与标量字段清单编号一致，且**必须给出清单里的全部编号**（1..N 一个都不能少，缺键＝漏填，会被打回重填）；某栏确实无依据就写「未提及」，不要省略该键
...
- **栏内形态（分点优先）**：并列同类事项（多主体、多事项、多步骤、多分项数据、清单类要点）**必须** `- ` 一条一行、标签嵌在行首（`- **标签**：内容`），不要把多条压进一句；**同一栏连续 3 行以上独占一行的 `**标签**：`（没有 `- `）算形态缺陷**——清单必须分点；关键数字、人名、结论用 `**加粗**`；原文关键表态/承诺/结论的原话用 `> 引用`（逐字）；不要为凑条目把一条长句硬拆成多行
...
- **禁止留空栏**：某栏在内容来源里确实无依据时，写约定的缺省词（「未提及」/「未明确」），不要留空、不要只留标题、不要省略该键（缺键＝漏填，会被打回重填）
...
- **未声明全文字数上限时，篇幅以原文为参照**：与原文同量级（90%–110%）即可，不必强压；按栏目把原文事实写清——宁可接近原文长度，也不丢原文已有的事实
- **冲突裁决**：**保真优先**（不删事实、不改口径与人名数字）→ 其次保结构（栏目齐全）→ 最后才压表达；篇幅以原文量级为参照，明显超量时先删套话与同义反复、再归并同类——**压缩不等于删事实**：数字、日期、人名、决策、承诺、模块进展与风险一律保留
- 「一段话概括 / 概况」= **完整信息段**（一般 4–10 句；信息少时一两句即合格），写清整体进展、关键里程碑/节点、主要风险或阻塞、下一步；必须带原文中的数字、地点、责任人；**禁止只有一句空泛总结**
