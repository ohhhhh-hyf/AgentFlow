...
- **未声明全文字数上限时，篇幅上限就是原文**：不得超过原文长度；按栏目把原文事实写清，但不为填满栏目扩写
- **冲突裁决**：**篇幅上限是硬约束（不得突破）**；在不超过上限的前提下，优先保真（不删事实、不改口径与人名数字），其次保结构（栏目齐全）；超量时先压缩表达、再归并同类
- 「一段话概括 / 概况」= **完整信息段**（最多 4–8 句；信息少时一两句即合格），写清整体进展、关键里程碑/节点、主要风险或阻塞、下一步；必须带原文中的数字、地点、责任人；**禁止只有一句空泛总结**
