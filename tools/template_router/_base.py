...
def _describe_field(index: int, seg: dict) -> str:
    ...
    if seg.get("heading"):
        ...
        if _HAS_SIZE_SPEC_RE.search(hint):
            ...
            desc += (
                "：**写 1–2 段完整概括，每段不超过 400 字**（若上下文给了【篇幅预算】的第 1 栏口径，"
...
- 「一段话概括 / 概况」= **最多 3 段、每段不超过 400 字**；只写该栏主题的概括（背景、目的、结论口径），可带 1–2 个关键数字作锚点；人物/数据清单、事项进展、承诺与待办等明细**归各自栏目，本栏不复述**；信息少时一两句即合格，**禁止只有一句空泛总结**
