...
- **栏内形态**：并列事实（多主体、多事项、多步骤、多分项数据）用 `- ` 一条一行，不要把多条压进一句；关键数字、人名、结论用 `**加粗**`；原文关键表态/承诺/结论的原话用 `> 引用`（逐字）；不要为凑条目把一条长句硬拆成多行
- **每条写实（宁足不短）**：一条一个事项或一个判断，但要把原文给的要素写足（主体 + 数字/时间/范围 + 结果或状态/影响 + 原文的其它限定）；**参考 30–100 字，信息多可更长**——不要把一条压成一句空话，也不要为凑短而砍要素；禁止无锚点空条（「讨论了X问题」「提到了Y」「进行了沟通」这类）
- **拆条只在必要时**：只有一条里并列了多个**互不相干**的事项时才拆成多条；拆后每条仍要写足要素，不要拆成短句清单
- **条数宁多不漏**：原文有几类、几项就写几条/几组，不要为了简短把多项合成一条（要点、风险、待决、证据、问答类栏目尤其如此）
- **加粗配额**：加粗只给关键数字、结论或专名，**每条 1–2 处（事实多的长条最多 3 处）**，不要整句加粗、不要同类词处处加粗
