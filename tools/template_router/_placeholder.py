...
def build_placeholder_fill_user(
    ...
    *,
    ...
    lines = [
        ...
        "有据才写；缺内容就写「未提及」（不要解释原因）；**键必须齐全**：fields 要给出清单里全部编号，缺键＝漏填。",
        "字段值与表格里不得复述、解释或引用模板要求（如「以上均未明确…填写『无』」）；缺内容只写约定的缺省词。",
        "勿照抄「如：」示例；勿张冠李戴；勿改数字；勿用百科补履历；勿虚构原文没有的内容。",
        "各栏按主题分别写清；「与/和/及」并列主题勿揉成一句糊涂话。",
        "栏内形态（分点优先）：并列同类事项（多主体、多事项、多步骤、多分项数据、清单类要点）**必须** `- ` 一条一行、标签嵌在行首（`- **标签**：内容`）；"
        "**同一栏连续 3 行以上独占一行的 `**标签**：`（没有 `- `）算形态缺陷**。"
        "关键数字、人名、结论用 **加粗**；原文关键表态/承诺/结论的原话用 > 引用（逐字）；"
        "不要为凑条目把一条长句硬拆成多行。",
        "**大类分组**：独占一行的 `**大类**：` 只用来把该栏分成 2–5 个大类，大类之下必须跟 `- ` 子条；"
        ...
        "**语音识别错（错字/专名/数字）**：能明确辨识的按公认写法写（如「结盟不对抗」→「不结盟不对抗」、「chat box TTS」→「ChatTTS」）；无法确定的照原文写；禁止「可能为X」「疑似X」式猜测补全。",
        "**禁止留空栏**：某栏在内容来源里确实无依据时，写约定的缺省词（「未提及」/「未明确」），不要留空、不要只留标题。",
        "**来源优先级：内容来源（原文/材料）> 已批准要点（草稿）> 索引类摘要**；引话、专名、过程、评价类细节回内容来源取，结论、数字口径与归属以已批准要点为准。",
        "简洁/粗略≠空洞：每栏写清该栏主要事实与要点，可多句。",
        "「一段话概括」不是一句空话：概况段按原文信息量成段（信息充足时 4–10 句、信息少时一两句即可），含整体进展、里程碑/节点、主要风险、下一步，并带原文数字/地点/责任人。",
