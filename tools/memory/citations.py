...
def _extract_source_index(context: str) -> dict[tuple[str, str, str], tuple[str, str]]:
    raw = context or ""
    marker = "【记忆来源索引】"
    ...
    for line in chunk.splitlines():
        ...
        at = _clean(match.group(4))
        out[_source_key(entity, kind, body)] = (title, at)
    return out
...
def extract_memory_references(context: str, *, limit: int = 24) -> list[MemoryReference]:
    ...
    for hit in parse_recall_from_text(context):
        ...
        for item in hit.get("history") or []:
            ...
            meeting_title, meeting_time = source_index.get(
...
def _extract_display_references(
    ...
    *,
    limit: int = 24,
    source_index: dict[tuple[str, str, str], tuple[str, str]] | None = None,
    ...
    for match in _DISPLAY_RE.finditer(markdown or ""):
        ...
        if not text:
            continue
        kind, body = _split_kind(text)
        meeting_title, meeting_time = (source_index or {}).get(
            _source_key(entity, kind, body),
            ("", ""),
        )
        marker = (entity, text)
...
def _best_span(line: str, ref: MemoryReference) -> tuple[int, int] | None:
    """在正文行里找记忆片段的精确锚点（宁缺毋滥，防误标）。

    规则（比旧实现更保守）：
    - 候选只取记忆文本里的完整分句（≥8 字）与长拉丁术语（≥6 字）；
      不再把 3 字实体短名当作锚点（正文出现同名短词 ≠ 内容来自记忆）。
    - 汉字回退匹配最小连续长度 6 字（旧 4 字），低于则放弃该行。
    - 找不到精确锚点返回 None（调用方不标注、不整行下划线）。
    """
    body = _clean(line)
    ref_text = _clean(ref.text)
    if not body or not ref_text:
        return None
    candidates: list[str] = []
    ...
    candidates.extend(
        token for token in _LATIN_TERM.findall(ref_text) if len(token) >= 6
    )
    ...
    left = "".join(ch for ch in line if "\u4e00" <= ch <= "\u9fff")
    right = "".join(ch for ch in ref_text if "\u4e00" <= ch <= "\u9fff")
    best = ""
    for size in range(min(18, len(right)), 5, -1):
    ...
    if not best:
        return None
    pattern = ".*?".join(re.escape(ch) for ch in best)
