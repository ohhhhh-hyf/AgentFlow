...
# 项目根（脚本位于 tools/codegen/sync_domain.py）
...
GEN_ZONE_START = "# ── 生成模型生成区：由 tools/codegen/sync_domain.py 生成，勿手改 ──"
GEN_ZONE_END = "# ── 生成模型生成区结束 ──"

# 空结构常量生成区标记（orchestrator.py）
GEN_ZONE_EMPTY_START = "# ── 空结构常量生成区：由 tools/codegen/sync_domain.py 生成，勿手改 ──"
    ...
    if kind == "enum":
        ...
        if field.get("normalize"):
            return (
                ...
                f'data["{key}"], {{{values}}}, "{field["normalize"]}")'
            )
...
def _referenced_empty_consts() -> set[str]:
    """域内 .py **真正引用**到的 _EMPTY_* 名字（供空结构常量按需生成）。

    必须排除"空结构常量生成区"里的定义行本身——否则每个常量都能"自己引用自己"，
    过滤形同虚设（2026-09-21 首版就栽在这：_EMPTY_CATALOG_SLIM 照样生成）。
    """
    names: set[str] = set()
    for path in CURRENT.dir.rglob("*.py"):
        try:
            text = path.read_text(encoding="utf-8")
        except OSError:  # noqa: PERF203 - 生成期只读，读不到就跳过
            continue
        text = re.sub(
            rf"{re.escape(GEN_ZONE_EMPTY_START)}.*?{re.escape(GEN_ZONE_EMPTY_END)}",
            "",
            text,
            flags=re.S,
        )
        for line in text.splitlines():
            if re.match(r"^_EMPTY_[A-Z][A-Z0-9_]*\s*=", line):  # 定义行（含域内自建常量）
                continue
            names.update(re.findall(r"\b_EMPTY_[A-Z][A-Z0-9_]*\b", line))
    return names
...
def gen_generate_all() -> tuple[str, str]:
    ...
    generated = []
    empty_consts = []
    # 空结构常量（_EMPTY_X）只生成"有人用"的：任务线的 empty_draft 要它，
    # 或域内代码显式引用了它（如核心理解的空值）。否则会产出 _EMPTY_CATALOG_SLIM
    # 这种与 _EMPTY_CATALOG 逐字重复、全仓零引用的死常量（2026-09-21 清理时确认）。
    line_bases = {_contract_base(line) for line in find_lines()}
    used_consts = _referenced_empty_consts()
    for contract in sorted(find_gen_contracts(), key=lambda c: c["cls"].__name__):
        ...
        if model_cls in line_bases or f"_EMPTY_{_upper_snake(model_cls)}" in used_consts:
...
SUP_ZONE_START = "# ── 审核模型生成区：由 tools/codegen/sync_domain.py 生成，勿手改 ──"
SUP_ZONE_END = "# ── 审核模型生成区结束 ──"

# 拒绝审核常量生成区标记（orchestrator.py）
SUP_ZONE_REJECT_START = "# ── 拒绝审核常量生成区：由 tools/codegen/sync_domain.py 生成，勿手改 ──"
...
def _task_readiness_issues(line: str) -> list[str]:
    ...
    if not prompts_path.exists():
        ...
        # 渲染提示词只在「该线的渲染步骤真的引用它」时才要求存在：deterministic_pipeline
        # 线（如 minutes_trace，llm_render=never）的正文由程序落钉，从不读这两个常量——
        # 原先一律要求，结果必须留两个空串常量凑齐（2026-09-21 清理）。
        render_path = task_dir / "steps" / f"{line}_render.py"
        render_src = render_path.read_text(encoding="utf-8-sig") if render_path.exists() else ""
        required_prompts += [
            name
            for name in (f"{upper}_RENDER_PROMPT", f"{upper}_RENDER_TEMPLATE_PROMPT")
            if name in render_src
        ]
...
def _print_readiness_issues(issues: list[str]) -> None:
    ...
    print(f"  python tools/codegen/sync_domain.py --domain {CURRENT.name}", file=sys.stderr)
...
def write_task_skels(lines: list[str]) -> None:
    ...
    - 任务线 ``__init__.py``：纯导出模板，**仅在该文件不存在时创建**（已存在则原样
      保留——用户会在里面加导出，整体重写会冲掉）。注意：因此历史遗留的别名导出不会
      被自动清掉，需要手工清理（2026-09-21 就是手工删了 12 个别名）。
    """
    for line in lines:
...
def check_core_understanding() -> int:
    ...
    for path, *needles in (
        ...
        if missing:
            ...
            rc = 1
    # 编排文件：理解节点允许两种形态（普通节点 / 工厂生成）**任一即可**
    # （meeting 用工厂 `_make_X_understanding_node`，notes 用普通节点），
    # 类名与图节点名仍必须出现。
    orch = CURRENT.orch_path
    text = orch.read_text(encoding="utf-8") if orch.exists() else ""
    node_forms = (info["node"], f'_make{info["node"]}')
    missing = [needle for needle in (info["class"], info["graph_node"]) if needle not in text]
    if not any(form in text for form in node_forms):
        missing.append(" 或 ".join(node_forms))
    if missing:
        _log(f"{orch.name} 缺少 core understanding 接线：{missing}", file=sys.stderr)
        rc = 1
    return rc
    ...
    try:
        ...
        if args.check:
            ...
            if rc != 0:
                ...
                print(
                    f"建议先运行：python tools/codegen/sync_domain.py --domain {args.domain}",
                    file=sys.stderr,
                )
                sys.exit(1)
