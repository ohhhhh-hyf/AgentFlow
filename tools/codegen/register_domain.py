...
2. 从 tools/codegen/domain_template/ 渲染骨架文件
...
def main() -> None:
    ...
    print(f"  2. python tools/codegen/sync_domain.py --domain {domain}")
    print(
        f'  3. python tools/codegen/register_task.py --domain {domain} '
        f'--task xxx --name "中文名"'
    )
    print("  4. 补齐 task 的 contracts/prompts/steps 和 reports.py")
    print(f"  5. python tools/codegen/sync_domain.py --domain {domain}")
    print(f"  6. python tools/codegen/sync_domain.py --domain {domain} --check")
