"""tools.templates.router.base —— 模板路由·基础层：常量与无依赖工具（判型/解析/计数/文本工具）。"""
...
# 栏位自己声明了尺寸（N字/N句/N段/N条/N行）→ 以其声明为准，不再叠加默认上限
_HAS_SIZE_SPEC_RE = re.compile(r"\d+\s*(?:字|句|段|条|行)")
...
# 方括号内是「请模型自拟」的指令（如「自主概括的议程模块名称」）→ 不是固定栏名，
# 标题由模型起/值写标题文字：`_section_title_match` 与 `_mark_title_fields` 共用。
_TITLE_HINT_INSTRUCTION_RE = re.compile(r"自主|概括|填写|根据|提取|写出|生成|待填")
...
# ── 表格栏说明 ────────────────────────────────────────────────────────────
# 背景（2026-09，now.xlsx 实测）：模板里"按下表逐行填写…"这类说明行紧跟着表格，
...
def _describe_field(index: int, seg: dict) -> str:
    ...
    if seg.get("missing"):
        desc += "：缺内容按该栏约定的缺省词填写（模板没约定时写「未提及」）；不要写说明句"
    # 概况判定只看**首个分句**：避免 hint 里顺带提到"摘要/与…对应"（指别的栏目）被误判成本栏要写成概况段；
    # 以「从/由/根据…」开头的句子说的是取材来源（如「从概况与原文提取下一步」），不是本栏要写成概况。
    lead = re.split(r"[。；;，,]", hint, maxsplit=1)[0][:24]
    if seg.get("heading"):
        # 标题行上的占位（如 `## [自主概括的议程模块名称]`）：值 = 标题文字，不是正文段落
        desc += "：只写该栏标题文字（短语；不要 # 前缀、不要句号、不要写成段落）"
    elif re.search(r"概括|概况|概览|综述|摘要", lead) and not re.match(
        r"[\s*]*(?:从|由|根据|依据|参考|结合)", lead
    ):
        # 概括/背景类栏位是最容易被灌满的位置（实测出现 2764 字单段 + 复述明细栏内容），
        # 因此这里逐栏给出默认上限与边界；栏位自己声明了尺寸的以模板为准（不叠加）。
        if _HAS_SIZE_SPEC_RE.search(hint):
            desc += "：按该栏说明写概括（明细归各自栏目，本栏不复述）；禁止单句空话"
        elif index == 1:
            # 第 1 栏＝总述栏（"只读一段"的入口）：只给上限治不了薄（实测首栏中位数约 150 汉字、
            # 最薄 88），所以这里给"完整概括"的口径与要素清单；具体下限由【篇幅预算】按原文规模
            # 算好（length_budget.first_column_min），不在此处重复数字以免两处漂移。
            # 2026-09-19 用户口径：首栏**只写一段**、≤400 字（总览不是正文，多段=变相展开）。
            desc += (
                "：**只写一段完整概括，不超过 400 字**（若上下文给了【篇幅预算】的第 1 栏口径，"
                "按其下限；原文事实不足时按实际写，不凑字）；交代清：① 谁/什么场合（人数、机构、身份照原文）"
                "② 围绕什么、覆盖哪几块（把名字列出来）③ 结论或基调（没有结论就写明"
                "「以分享为主，未形成明确结论」）④ 原文有关键数字时选 1–3 个作锚点，没有则不强求；"
                "人物/数据清单、事项进展、承诺与待办等明细**归各自栏目，本栏不复述**；禁止单句空话"
            )
        else:
            desc += (
                "：**最多 3 段、每段不超过 400 字**；只写本栏主题的概括（背景、目的、结论口径），"
                "可带 1–2 个关键数字作锚点；人物/数据清单、事项进展、承诺与待办等明细"
                "**归各自栏目，本栏不复述**；信息多时优先精简本栏，把内容放到对应的栏；禁止单句空话"
            )
    elif index == 1 and not _HAS_SIZE_SPEC_RE.search(hint):
        # 第 1 栏＝总述栏，但它自称"一段话交代…"（就医/宣讲/研讨这类已自列要素的模板）：
        # 不给会议专用的四要素（如"没有结论就写以分享为主"对就诊/咨询语义不对），只补下限与边界。
        desc += (
            "：**只写一段完整概括，不超过 400 字**（若上下文给了【篇幅预算】的第 1 栏口径，"
            "按其下限；原文事实不足时按实际写，不凑字）；按本栏说明把要素交代完整，"
            "关键数字只写原文有的；人物/数据清单、事项进展、承诺与待办等明细"
            "**归各自栏目，本栏不复述**；禁止单句空话"
        )
...
def _char_budget_lines(template: str) -> list[str]:
    ...
    if sections:
        bits = [
            f"「{item['title']}」"
            f"{'每段约' if item.get('scope') == 'paragraph' else '本栏约'} {item['hi']} 字"
...
## 输出约定
1. fields 的 key 为字符串数字，与标量字段清单编号一致，且**必须给出清单里的全部编号**（1..N 一个都不能少，缺键＝漏填，会被打回重填）；某栏确实无依据就写该栏约定的缺省词（模板没约定时写「未提及」），不要省略该键
...
- **未声明栏位字数上限时，服从上下文【篇幅预算】**；没有动态预算时按事实量自然展开，不按原文比例机械扩写。单栏/单段上限见形态规则与字段清单，内容多靠增加段数与分条承载，不是把一段写长
- **冲突裁决**：模板显式栏位上限 > 动态总预算 > 默认形态规则；在有效预算内，**保真优先**（不删事实、不改口径与人名数字）→ 其次保结构（栏目齐全）→ 最后才压表达。超量时先删套话与同义反复，再按主题归组但保留组内子项——**压缩不等于删事实**：数字、日期、人名、决策、承诺、模块进展与风险一律保留
- 「一段话概括 / 概况」= **最多 3 段、每段不超过 400 字**；**总述栏（首栏）只写一段、不超过 400 字**；只写该栏主题的概括（背景、目的、结论口径），原文有关键数字时可带 1–2 个作锚点，没有则不强求；人物/数据清单、事项进展、承诺与待办等明细**归各自栏目，本栏不复述**；信息少时一两句即合格，**禁止只有一句空泛总结**
- 先按各栏主题写全实质内容，再对照各自约束调节；不要用某一段的字数去压缩其它段或表格
- 当内容多于用户要求：先保留结论、数字、负责人、期限、明确风险/行动等高价值信息，删去寒暄、重复、背景铺垫、低确定性猜测和不影响结论的枝节；压缩后仍要语句完整。
- 当内容少于用户声明的字数/条数要求：可以把原文中已出现的相关事实稍作展开、合并上下文说清楚，但**绝对不能编造**原文没有的事实、数字、责任人、期限或评价。
...
- 仅当来源对该栏主题完全无信息时，才写该栏约定的缺省词（模板没约定时写「未提及」）
...
- 候选项少于 N 行时，不要为了凑行数编造；只输出有依据的行，必要时一行写该栏约定的缺省词（模板没约定时写「未提及」）。
"""
)
