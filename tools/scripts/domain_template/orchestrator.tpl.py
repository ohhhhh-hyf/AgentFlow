...
def _line(state: {{STATE_CLASS}}, line_name: str) -> dict:
...
def _line_template(state: {{STATE_CLASS}}, line_name: str) -> str:
...
def _compute_title(state: {{STATE_CLASS}}) -> str:
...
def _empty_purpose(state: {{STATE_CLASS}}) -> str:
    """empty_purpose 兜底时的「目的」文案（领域有核心理解时覆写）。"""
    return ""


def _fallback_text(
    state: {{STATE_CLASS}}, line_name: str, rules
) -> tuple[str, list | None]:
...
class _Nodes:
    """图节点实现（mixin，供 {{PASCAL}}AgentSystem 继承）。
    ...
    @staticmethod
    def _mode_label(state: {{STATE_CLASS}}) -> str:
        return "objective" if state.get("objective_perspective") else "personal"

    @staticmethod
    def _shared_context(state: {{STATE_CLASS}}) -> str:
    ...
    def _supervisor_context(self, state: {{STATE_CLASS}}, line_name: str) -> str:
    ...
    def _make_agent_node(self, line_name: str):
        ...
        async def node(state: {{STATE_CLASS}}) -> dict:
    ...
    def _make_route(self, line_name: str):
        ...
        def route(state: {{STATE_CLASS}}) -> str:
            review = _line(state, line_name).get("review") or {}
    ...
    def _line_title(self, state: {{STATE_CLASS}}, line_name: str) -> str:
    ...
    def _fallback_reports(
        self, state: {{STATE_CLASS}}, line_names: list[str]
    ) -> dict:
