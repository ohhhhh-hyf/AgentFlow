...
def _root(tmp: Path) -> Path:
    ...
    profiles = tmp / "perspective" / "profiles"
...
def test_sanitize_and_merge(tmp: Path) -> None:
    ...
    # 职业文件本身不受清洗影响
    dev_raw = json.loads((root / "perspective" / "profiles" / "developer.json").read_text(encoding="utf-8"))
    check("职业文件仍按职业模板分类（清洗只作用 user.json）",
          classify_profile(resolve_role_template(dev_raw, root / "perspective" / "profiles")) == "role_template", "")
    check("is_user_profile_file 只认 user.json",
          is_user_profile_file(ui) and not is_user_profile_file(root / "perspective" / "profiles" / "developer.json"), "")
    ...
    obj_identity = MeetingIdentity(**filter_identity_fields(
        json.loads((root / "perspective" / "profiles" / "object.json").read_text(encoding="utf-8")),
        MeetingIdentity,
    ))
