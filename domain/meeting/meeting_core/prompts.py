...
topics：一个独立议题一个 topic；**同一议题多次出现合并成一条，但同一议题下的不同子事项、子话题分别保留**（宁多一条，不要并成一句）；title 可作分支名；discussion **按信息点分行写（一条一行）**，把该议题的事实写充实——关键数字与指标、对照取值、金额、日期与时限、人名、范围边界、分歧、结论线索、推进状态都要落进对应行，不要把多条信息压成一句、也不要写成概括性的一句空话；conclusion 有明确结论才填，否则 null；participants 只写真实名。
