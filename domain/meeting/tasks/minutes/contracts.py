...
class MinutesGenerationContract(GenerationContract):
    ...
    fields = [
        ...
        StrListField(
            "executive_summary",
            "概述（数组每项 = 一整段正文，不超过 3 句或约 200 字，不是标题、不是一句话。"
        ...
        StrListField(
            ...
            "**真人模式**（以【本用户命中】里他的待办为准）：自己的条目排在**最前**、"
            "**不写自己的姓名**（归属由渲染层的「我的事项」组名承担）；"
            "他人的分工**按其姓名分组**写在后面、**每条以「姓名：」开头**；"
