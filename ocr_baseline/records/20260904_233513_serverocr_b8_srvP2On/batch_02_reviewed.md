...
$$\hat{L}_x = \langle l m | \hat{L}_x | l m \rangle = \frac{1}{i\hbar} \langle l m | [\hat{L}_y, \hat{L}_z] | l m \rangle = \frac{1}{i\hbar} [\langle l m | \hat{L}_y \hat{L}_z | l m \rangle - \langle l m | \hat{L}_z \hat{L}_y | l m \rangle]$$

由于 $\hat{L}_z |l m\rangle = m\hbar |l m\rangle$，$m$ 为实数，则 $\langle l m | \hat{L}_z = m\hbar \langle l m |$，代入得：

$$\hat{L}_x = \frac{1}{i\hbar} [m\hbar \langle l m | \hat{L}_y | l m \rangle - m\hbar \langle l m | \hat{L}_y | l m \rangle] = 0$$

同理可得 $\hat{L}_y = 0$。

**例2.** $|l m\rangle$ 为 $(\hat{L}^2, \hat{L}_z)$ 的共同本征态，求 $\overline{\hat{L}_x^2}$，$\overline{\hat{L}_y^2}$。

$$\overline{\hat{L}_x^2} = \langle l m | \hat{L}_x^2 | l m \rangle = \langle l m | (\hat{L}^2 - \hat{L}_z^2) | l m \rangle = [l(l+1) - m^2]\hbar^2$$

$$\overline{\hat{L}_y^2} = \langle l m | \hat{L}_y^2 | l m \rangle = \langle l m | (\hat{L}^2 - \hat{L}_z^2) | l m \rangle = [l(l+1) - m^2]\hbar^2$$

利用 $[\hat{L}_x, \hat{L}_y] = i\hbar \hat{L}_z$，可得：

$$\hat{L}_x \hat{L}_y = \frac{1}{2} ([\hat{L}_x, \hat{L}_y] + \{\hat{L}_x, \hat{L}_y\}) = \frac{i\hbar}{2} \hat{L}_z + \frac{1}{2} (\hat{L}_x \hat{L}_y + \hat{L}_y \hat{L}_x)$$

$$\overline{\hat{L}_x \hat{L}_y} = \frac{i\hbar}{2} \langle l m | \hat{L}_z | l m \rangle + \frac{1}{2} \langle l m | (\hat{L}_x \hat{L}_y - \hat{L}_y \hat{L}_x) | l m \rangle = \frac{i\hbar}{2} m\hbar + \frac{1}{2} \langle l m | [\hat{L}_x, \hat{L}_y] | l m \rangle$$

$$= \frac{i m \hbar^2}{2} + \frac{i\hbar}{2} \langle l m | \hat{L}_z | l m \rangle = \frac{i m \hbar^2}{2} + \frac{i m \hbar^2}{2} = i m \hbar^2$$

故 $\overline{\hat{L}_x^2} = \overline{\hat{L}_y^2} = \frac{1}{2} [l(l+1) - m^2]\hbar^2$。
...
$$\Delta A = \sqrt{\langle \hat{A}^2 \rangle - \langle A \rangle^2}, \quad \Delta B = \sqrt{\langle \hat{B}^2 \rangle - \langle B \rangle^2}$$
...
由 $\langle \phi | \phi \rangle \ge 0$，得 $\xi^2 \langle \hat{x}^2 \rangle + \langle \hat{y}^2 \rangle + i\xi \langle [\hat{x}, \hat{y}] \rangle \ge 0$，即 $\langle \hat{x}^2 \rangle \cdot \langle \hat{y}^2 \rangle \ge \frac{1}{4} |\langle [\hat{x}, \hat{y}] \rangle|^2$。
...
证明：$[\hat{A}, \hat{B}] = 0 \Rightarrow \hat{A}\hat{B} = \hat{B}\hat{A}$。$\hat{B}\hat{A}|k\rangle = \hat{B} \cdot a_k |k\rangle = a_k \hat{B}|k\rangle$。则 $\hat{A}(\hat{B}|k\rangle) = a_k (\hat{B}|k\rangle)$，故 $\hat{B}|k\rangle$ 也是 $A$ 属于本征值 $a_k$ 的本征态。故 $\hat{B}|k\rangle$ 与 $|k\rangle$ 是同一个量子态，即 $\hat{B}|k\rangle = b_k |k\rangle$。$\Rightarrow A, B$ 拥有共同本征态 $|k\rangle$。

**例**：$\psi_p(\vec{r}) = \frac{1}{(2\pi\hbar)^{3/2}} e^{i\vec{p}\cdot\vec{r}/\hbar}$ 为 $\hat{p}_x, \hat{p}_y, \hat{p}_z$ 的共同本征函数，本征值为 $p_x, p_y, p_z$。$\hat{p} \psi_p(\vec{r}) = \frac{\hbar}{i} \nabla \psi_p(\vec{r}) = \vec{p} \psi_p(\vec{r})$。
...
**定理**：厄米算符 $\hat{A}$ 在任意量子态下的平均值 $\bar{A}$ 为实数，$\hat{A}^2$ 的平均值 $\overline{A^2} \ge 0$。

$$\langle \psi | \hat{A} | \psi \rangle = \langle \psi | \hat{A}^\dagger | \psi \rangle = \langle \psi | \hat{A} \cdot | \psi \rangle = \langle \psi^* | \hat{A} | \psi^* \rangle = (\langle \psi | \hat{A} | \psi \rangle)^*$$

因 $\hat{A} = \hat{A}^\dagger$，$\bar{A}$ 为实数。

或：$\langle \psi | \hat{A}^\dagger | \psi \rangle = \langle \hat{A} \psi | \psi \rangle$，$\langle \psi | \hat{A} | \psi \rangle = \langle \hat{A} \psi | \psi \rangle$，$\hat{A} = \hat{A}^\dagger = \hat{A}^*$，$\bar{A}$ 为实数。

$$|\psi\rangle = \hat{A} |\psi\rangle$$

$$\langle \psi | \hat{A}^2 | \psi \rangle = \langle \psi | \hat{A} \hat{A} | \psi \rangle = \langle \psi | \hat{A}^\dagger \hat{A} | \psi \rangle = \langle \hat{A} \psi | \hat{A} \psi \rangle = \langle \psi | \psi \rangle \ge 0, \quad \overline{A^2} \ge 0$$
...
**定理**：哈密顿算符 $\hat{H}$ 为厄米算符，满足本征方程 $\hat{H}|k\rangle = E_k |k\rangle$，对体系的任一归一化态 $|\psi\rangle$，若 $\bar{H} = \langle \psi | \hat{H} | \psi \rangle$ 有下界（总大于某常数）但无上界，则 $\hat{H}$ 的本征态 $|k\rangle$ 的集合构成体系的一个**完备集**，即体系的任一量子态 $|\psi\rangle$ 可用 $|k\rangle$ 来展开。
...
缺失段前文与后文之间没有需要补回的 OCR 行，因为所列三行均为页眉或页脚残留（如“UNIVERSITY OF SCIENCE AND TECHNOLOGY”“R.China 中国．武汉”），属于明显噪声，不应输出。因此，原稿内容保持连续，无需插入任何片段。
...
设 $F=(A_1, A_2, \dots, A_n)$ 是一组力学量完全集，$|k\rangle$ 是它们的共同本征态，其中 $k$ 表征所有量子数。$|k\rangle$ 是正交归一的，满足 $\langle k | m \rangle = \delta_{km}$（$=1$ 当 $k=m$，$=0$ 当 $k \neq m$）。$|k\rangle$ 是完备的，即体系中任一量子态可按 $|k\rangle$ 展开。
...
在 $F$ 表象中，$|k\rangle$ 为基矢，$|\psi\rangle = \sum_k a_k |k\rangle$，$|\psi\rangle$ 在 $F$ 表象中可用系数列向量表示为 $a = (a_k)$。

在另一 $F'$ 表象中，$|\beta\rangle$ 为基矢，$|\psi\rangle = \sum_\beta a_\beta' |\beta\rangle$，在 $F'$ 表象中可表示为 $a' = (a_\beta')$。

$a'$ 与 $a$ 的转化实际上是基矢之间的转化。

$|\psi\rangle = \sum_k a_k |k\rangle = \sum_\beta b_\beta |\beta\rangle$，将 $|k\rangle$ 按 $|\beta\rangle$ 展开：$|k\rangle = \sum_\beta \langle \beta | k \rangle |\beta\rangle$，从而统一基矢为 $|\beta\rangle$。

记 $S_{\beta k} = \langle \beta | k \rangle$，$S = (S_{\beta k})$。

则 $a_\beta' = \sum_k a_k \langle \beta | k \rangle$，即 $a' = S a$。

$S^\dagger S = S S^\dagger = 1$，即 $S$ 为**么正矩阵**。

> **注**：$a$、$a'$ 均为系数列向量，与本征态形式相似但意义完全不同。$|k\rangle$ 在希尔伯特空间中可表示为一个列向量，$|\psi\rangle$ 按 $|k\rangle$ 展开所得系数列为 $a$。

从矢量的角度考虑，$|\psi\rangle = (e_1, e_2, \dots, e_k, \dots)$ 列向量 $e_i'$ 的线性组合系数列。要"消去" $(e_1', e_2', \dots, e_k', \dots)$，则

$$(e_1, e_2, \dots, e_k, \dots) = (e_1', e_2', \dots, e_k', \dots) S$$

其中

$$S = \begin{pmatrix} S_{11} & S_{12} & \dots & S_{1k} & \dots \\ S_{21} & S_{22} & \dots & S_{2k} & \dots \\ \vdots & \vdots & & \vdots & \\ S_{k1} & S_{k2} & \dots & S_{kk} & \dots \end{pmatrix}$$

由 $\langle k' | k \rangle = \delta_{k'k}$，$\langle k | k \rangle = 1$，$|k\rangle = \sum_\beta S_{\beta k} |\beta\rangle$，得
...
$$(e_1, e_2, \dots)(e_1, e_2, \dots)^\dagger = (e_1, e_2, \dots) S S^\dagger (e_1, e_2, \dots)^\dagger$$

$$= (e_1, e_2, \dots) S (e_1, e_2, \dots)^\dagger$$
