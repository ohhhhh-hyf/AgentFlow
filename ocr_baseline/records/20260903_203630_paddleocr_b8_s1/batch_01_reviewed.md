...
且 $\cot ka<0$。$E$ 满足超越方程 $E=\frac{\hbar^2}{2\mu a^2}\left[1-\cos(2ka)\right]$ 且 $\cot(ka)<0$。能量是量子化的。
...
分子线度为 $d$，则 $a^3 \ll V$，$d \ll \bar{l}$，即可不计分子本身的大小。
...
设 $A_1$ 为第 $i$ 个分子与 $A_1$ 碰撞，$y,z$ 分量不变，$x$ 方向速度分量由 $v_{ix}\to -v_{ix}$。

$$P=\frac{2mv_{ix}}{\Delta t}\times\frac{N}{abc}=\frac{2}{3}n\bar{\varepsilon}$$

$v^2=\frac{3kT}{m}$，$\bar{\varepsilon}=\frac{1}{2}m\overline{v^2}=\frac{3}{2}kT$

$E_k=\frac{3}{2}kT$ 为粒子的平均动能。

$$\therefore P=\frac{2}{3}n\bar{\varepsilon}=nkT \Rightarrow \bar{\varepsilon}=\frac{3}{2}kT$$

## 分子的平均动能与理想气体系统的内能

只考虑分子平动动能，$U=N\bar{\varepsilon}=\frac{3}{2}NkT=\frac{3}{2}RT$。
...
以 $x、P$ 为直角坐标可构成二维 $\mu$ 空间，若 $\varepsilon$ 给定，则代表点的轨迹是椭圆，椭圆面积 $=\pi\sqrt{\frac{2\varepsilon}{m\omega^2}}\sqrt{2m\varepsilon}$。
...
$I_{总}=m_1r_1^2+m_2r_2^2=\mu r^2$，系统转动能量 $T=\frac{1}{2}m_1r_1^2(\dot{\theta}^2+\sin^2\theta\dot{\varphi}^2)+\frac{1}{2}m_2r_2^2(\dot{\theta}^2+\sin^2\theta\dot{\varphi}^2)$。

即 $T=\frac{1}{2}\mu r^2(\dot{\theta}^2+\sin^2\theta\dot{\varphi}^2)=\frac{1}{2}I(\dot{\theta}^2+\sin^2\theta\dot{\varphi}^2)$。

$P_\theta=\mu r^2\dot{\theta}=P_{\theta_1}+P_{\theta_2}$，$P_\varphi=\mu r^2\sin^2\theta\dot{\varphi}$（与 $\theta、\varphi$ 共轭的动量）。

$$T=\frac{1}{2}\mu r^2\dot{\theta}^2+\frac{1}{2}\mu r^2\sin^2\theta\dot{\varphi}^2=\frac{P_\theta^2}{2I}+\frac{P_\varphi^2}{2I\sin^2\theta}$$

## 宏观分布、组态、微观态

### μ空间分割（宏观）

把 $\mu$ 空间分成许多小体元 $\Delta\mu_j$（$j=1,2,\cdots,l$），$\Delta\mu_j$ 大小适当，足够小到可近似认为代表点落在 $\Delta\mu_j$ 内的粒子运动状态相同，但也不是无穷小，要满足 $a_j\gg 1$（统计需要）。
...
### 经典力学中一个组态的微观数（相字 $\Delta\mu_j\to$ 相格 $h^r$，过渡到微观）
...
子相字空间小体元 $\Delta\mu_j$（$j=1,2,\cdots,l$）中粒子运动状态数为 $\Delta\omega_j=\frac{\Delta\mu_j}{h_0^r}$，$a_j$ 个粒子在 $\Delta\omega_j$ 个运动状态上分布的可能微观状态数为 $\binom{\Delta\omega_j+a_j-1}{a_j}$，多个粒子可以处于同一相格内。

### 一个分布的微观分布数

$$\Omega=\prod_j \frac{\Delta\omega_j^{a_j}}{a_j!}$$
...
$$\delta(\ln\Omega)=\delta\left(\ln N!-\sum_j\ln a_j!\right)+\sum_j a_j\ln\Delta\omega_j=\sum_j(\ln\Delta\omega_j-\ln a_j)\delta a_j=0$$

由斯特林公式，$N$ 足够大时，有 $\ln N!\approx N\ln N-N$。

则 $\delta\sum_j(a_j\ln a_j-a_j-a_j\ln\Delta\omega_j)=\sum_j(\ln a_j+1-1-\ln\Delta\omega_j)\delta a_j=\sum_j(\ln a_j-\ln\Delta\omega_j)\delta a_j=0$。

又有粒子数和能量守恒条件（孤立），则 $\sum_j\delta a_j=0$，$\sum_j\varepsilon_j\delta a_j=0$。

令 $f(a_1,a_2,\cdots,a_l,\alpha,\beta)=\sum_j a_j(\ln a_j-1-\ln\Delta\omega_j)+\alpha\left(\sum_j a_j-N\right)+\beta\left(\sum_j\varepsilon_j a_j-E\right)$。

则 $f$ 取极值时，$\frac{\partial f}{\partial a_j}=0$，$\ln a_j-\ln\Delta\omega_j+\alpha+\beta\varepsilon_j=0$。

$$a_j=\Delta\omega_j e^{-\alpha-\beta\varepsilon_j}$$

且此时 $f$ 取极大值。

$\beta=\frac{1}{kT}$，$kT$ 是一个普遍量，$a_j=\Delta\mu_j e^{-\alpha-\beta\varepsilon_j}$。

$\alpha$：由 $\sum a_j=N$ 确定，$\frac{\partial \alpha}{\partial N}=0$，$\Omega=\frac{N!}{\prod a_j!}$。
...
$$W=\frac{(2n)!}{(n+\Delta n)!(n-\Delta n)!},\quad \frac{W}{W_m}=\frac{(n!)^2}{(n+\Delta n)!(n-\Delta n)!}\to 0，n 很大$$
...
对于一个自由度为 $r$ 的粒子，它的 μ空间中大小为 $h^r$ 的相体积对应一个量子态。

$0\sim p$ 范围内的总量子态数 $\Omega(p)=\frac{4\pi}{3}\frac{V}{h^3}p^3=\frac{4\pi V}{3h^3}(2\mu E)^{3/2}$。
...
内积：$(\varphi,\psi)=\langle\varphi|\psi\rangle$，坐标表象 $\int_{-\infty}^{\infty}\varphi^*(r)\psi(r)d\tau$，$d\tau=dxdydz$ 为微体积元，$\psi(r)=\psi(x,y,z)$。
...
$\hat{A}|\psi\rangle=|\hat{A}\psi\rangle$，在右矢空间中，$|\psi\rangle=|\psi\rangle=|\hat{A}\psi\rangle$，默认 $\hat{A}$ 向右作用。在左矢空间中，$\langle\psi|=\langle\psi|\hat{A}$，算符 $\hat{A}^\dagger(\hat{A}^T)$ 向左作用。$\langle\psi|\hat{A}^\dagger=\langle\hat{A}\psi|=\langle\psi|$。若 $\hat{A}$ 为厄米算符，则 $\langle\psi|\hat{A}=\langle\hat{A}\psi|$。

**厄米算符在左、右矢空间中的运算具有形式不变性。**

算符在左矢和右矢之间的转换本质上是对偶空间的伴随映射。

关于 $(\langle\psi|\hat{A})^\dagger=\hat{A}^\dagger|\psi\rangle$：$\bar{A}=\langle\psi|\hat{A}|\psi\rangle$，$\bar{A}^*=\langle\psi|\hat{A}^\dagger|\psi\rangle$，$\langle\psi|\hat{A}|\psi\rangle^*=\langle\psi|\hat{A}^\dagger|\psi\rangle$。

$\langle\psi|\hat{A}\hat{B}|\psi\rangle^*=\langle\psi|\hat{B}^\dagger\hat{A}^\dagger|\psi\rangle$（先将 $\hat{B}|\psi\rangle$ 视为一体，再作变换）。
...
$[\hat{l}_x,\hat{z}]=[\hat{y}\hat{p}_z-\hat{z}\hat{p}_y,\hat{z}]=[\hat{y}\hat{p}_z,\hat{z}]=-i\hbar\hat{y}$（$\hat{x}\hat{e}_y\hat{e}_z=-\hat{e}_y$）
...
$[\hat{l}_x,\hat{p}_y]=[\hat{y}\hat{p}_z-\hat{z}\hat{p}_y,\hat{p}_y]=[\hat{y}\hat{p}_z,\hat{p}_y]=[\hat{y},-i\hbar\frac{\partial}{\partial y}]\hat{p}_z=i\hbar\hat{p}_z$
...
令 $\hat{l}^2=\hat{l}_x^2+\hat{l}_y^2+\hat{l}_z^2=\hat{y}^2\hat{p}_z^2+\hat{z}^2\hat{p}_y^2-\hat{y}\hat{p}_z(\hat{z}\hat{p}_y)-\hat{z}\hat{p}_y(\hat{y}\hat{p}_z)$

则 $\hat{l}^2=(\hat{x}^2+\hat{y}^2)\hat{p}_z^2+(\hat{y}^2+\hat{z}^2)\hat{p}_x^2+(\hat{y}^2+\hat{z}^2)\hat{p}_y^2-(\hat{y}\hat{z}\hat{p}_z\hat{p}_y)-(\hat{y}\hat{z}\hat{p}_y\hat{p}_z)-(\hat{z}\hat{x}\hat{p}_z\hat{p}_x)$

$-(\hat{x}\hat{z}\hat{p}_x\hat{p}_z)-(\hat{x}\hat{y}\hat{p}_x\hat{p}_y-i\hbar\hat{x}\hat{p}_x)-(\hat{y}\hat{x}\hat{p}_y\hat{p}_x)$

## 对易式运算规则

$[\hat{A},\hat{B}\hat{C}]=\hat{A}\hat{B}\hat{C}-\hat{B}\hat{C}\hat{A}=\hat{A}(\hat{B}\hat{C}-\hat{C}\hat{B})+(\hat{A}\hat{C}\hat{B}-\hat{C}\hat{A}\hat{B})+\hat{C}\hat{A}\hat{B}-\hat{B}\hat{C}\hat{A}=\hat{A}[\hat{B},\hat{C}]+[\hat{A},\hat{C}]\hat{B}$

$\hat{B}$ 左移变边缘，可提公因式。

$[\hat{A}\hat{C},\hat{B}]=(\hat{A}\hat{B}-\hat{B}\hat{A})\hat{C}+\hat{B}\hat{A}\hat{C}-\hat{B}\hat{C}\hat{A}=[\hat{A},\hat{B}]\hat{C}+\hat{B}[\hat{A},\hat{C}]$

$$[\hat{A},\hat{B}\hat{C}]=[\hat{A},\hat{B}]\hat{C}+\hat{B}[\hat{A},\hat{C}]$$

$$[\hat{A}\hat{B},\hat{C}]=\hat{A}[\hat{B},\hat{C}]+[\hat{A},\hat{C}]\hat{B}$$

$[\hat{l}_x,\hat{l}^2]=[\hat{l}_x,\hat{l}_x^2]+[\hat{l}_x,\hat{l}_y^2]+[\hat{l}_x,\hat{l}_z^2]=0$，同理 $[\hat{l}_a,\hat{l}^2]=0$。
...
即 $[\hat{l}_a,\hat{l}_b]=i\hbar\varepsilon_{abc}\hat{l}_c$（$a,b,c$ 循环取 $x,y,z$）。

又因为 $\hat{l}^2=\hat{l}_x^2+\hat{l}_y^2+\hat{l}_z^2$，利用对易式恒等式：

$$[\hat{A}\hat{B},\hat{C}]=\hat{A}[\hat{B},\hat{C}]+[\hat{A},\hat{C}]\hat{B}$$

可得：

$$[\hat{l}_x,\hat{l}^2]=[\hat{l}_x,\hat{l}_x^2]+[\hat{l}_x,\hat{l}_y^2]+[\hat{l}_x,\hat{l}_z^2]=0$$

同理 $[\hat{l}_a,\hat{l}^2]=0$（$a=x,y,z$）。