...
**例1.** 求 $\hat{x}$ 和 $\hat{p}$ 在 $|l m\rangle$ 下的平均值。

已知：
$$\hat{L}_z |l m\rangle = m\hbar |l m\rangle, \quad [\hat{L}_y, \hat{L}_z] = i\hbar \hat{L}_x, \quad [\hat{L}_z, \hat{L}_x] = i\hbar \hat{L}_y$$

从而可用含 $\hat{L}_y$ 的表达式表出 $\hat{L}_x$、$\hat{L}_y$。
...
由于 $\hat{L}_z | l m \rangle = m\hbar | l m \rangle$，$m$ 为实数，则：
$$\langle l m | \hat{L}_z \hat{L}_y | l m \rangle = m\hbar \langle l m | \hat{L}_y | l m \rangle$$
...
$$\hat{L}_y \hat{L}_x = \hat{L}_x \hat{L}_y - i\hbar \hat{L}_z$$

$$\langle \hat{L}_x^2 \rangle = \langle l m | \hat{L}_x^2 | l m \rangle = \frac{1}{2} \langle l m | (\hat{L}^2 - \hat{L}_z^2) | l m \rangle = \frac{1}{2} [l(l+1) - m^2]\hbar^2$$

同理：
$$\langle \hat{L}_y^2 \rangle = \frac{1}{2} [l(l+1) - m^2]\hbar^2$$
...
其中 $[\hat{A}, \hat{B}] = \langle \psi | [\hat{A}, \hat{B}] | \psi \rangle$ 对 $|\psi\rangle$ 成立，$\Delta A = \hat{A} - \bar{A}$，$\Delta B = \hat{B} - \bar{B}$。
...
考虑 $|\phi\rangle = (\xi \hat{x} + i\hat{y})|\psi\rangle$，$\langle \phi | \phi \rangle \ge 0$，

$$\langle \psi | (\xi \hat{x} - i\hat{y})(\xi \hat{x} + i\hat{y}) | \psi \rangle = (\xi \langle \psi | \hat{x} - i \langle \psi | \hat{y})(\xi \hat{x} | \psi \rangle + i \hat{y} | \psi \rangle)$$
...
$$\therefore \Delta A \cdot \Delta B \ge \frac{1}{2} |\langle [\hat{A}, \hat{B}] \rangle|$$
...
**转置算符**：对 $\hat{A}$ 和 $\hat{B}$，若 $\langle \psi|\hat{A}|\phi\rangle = \langle \phi|\hat{A}|\psi\rangle^*$，则称 $\hat{A}$ 和 $\hat{A}^T$ 互为彼此的转置算符。
...
$$\langle \psi|\hat{A}|\psi\rangle = \langle \psi|\hat{A}^\dagger|\psi\rangle = \langle \psi|\hat{A}^T|\psi\rangle = \langle \psi^*|\hat{A}|\psi^*\rangle = (\langle \psi|\hat{A}|\psi\rangle)^*$$

因 $\hat{A} = \hat{A}^\dagger$，$\bar{A}$ 为实数。

或：$\langle \psi|\hat{A}^\dagger|\phi\rangle = \langle \hat{A}\psi|\phi\rangle$，$\langle \psi|\hat{A}|\phi\rangle = \langle \hat{A}\psi|\phi\rangle$，$\hat{A} = \hat{A}^\dagger = \hat{A}^T$，$\bar{A}$ 为实数。

$$|\phi\rangle = \hat{A}|\psi\rangle$$

$$\langle \psi|\hat{A}^2|\psi\rangle = \langle \psi|\hat{A}\hat{A}|\psi\rangle = \langle \psi|\hat{A}^\dagger\hat{A}|\psi\rangle = \langle \hat{A}\psi|\hat{A}\psi\rangle = \langle \phi|\phi\rangle \geq 0, \quad \bar{A^2} \geq 0$$

### 厄米算符本征值的实数性

$$\hat{F}|k\rangle = \lambda_k |k\rangle$$

则 $\langle k|\hat{F}|k\rangle = \bar{F} = \langle k|\lambda_k |k\rangle = \lambda_k \langle k|k\rangle = \lambda_k$，因 $\hat{F}$ 为厄米算符，$\lambda_k = \bar{F}$ 为实数。
...
缺失段前文与后文之间没有需要补回的 OCR 行，因为所列三行均为页眉或地址残留（如“UNIVERSITY OF SCIENCE AND TECHNOLOGY”“R.China 中国·武汉”），属于明显噪声，不应输出。因此，原稿在“能级简并”之后直接衔接“表象变换与矩阵力学”即可，无需插入任何内容。
...
$\frac{d\bar{A}}{dt} = \frac{\partial}{\partial t}\left(\langle\psi|\hat{A}|\psi\rangle + \langle\psi|\frac{\partial \hat{A}}{\partial t}|\psi\rangle + \langle\psi|\hat{A}\frac{\partial}{\partial t}|\psi\rangle\right) = -\frac{1}{i\hbar}\langle\psi|\hat{A}\hat{H}|\psi\rangle + \langle\psi|\frac{\partial \hat{A}}{\partial t}|\psi\rangle + \frac{1}{i\hbar}\langle\psi|\hat{H}\hat{A}|\psi\rangle$
...
设 $F=(A_1,A_2,\dots,A_n)$ 是一组力学量完全集，$|k\rangle$ 是共同本征态，其中 $k$ 表征所有量子数。
...
$F$ 表象中，$|k\rangle=\psi_k$，$|\psi\rangle=\sum_k a_k|k\rangle$，$|\psi\rangle$ 在 $F$ 表象中可用系数列向量表示为 $a=\begin{pmatrix}a_1\\a_2\\\vdots\end{pmatrix}$。

另一 $F'$ 表象中，$|\beta\rangle=\psi_\beta$ 为基矢，$|\psi\rangle=\sum_\beta a_\beta|\beta\rangle$，在 $F'$ 表象中可表示为 $a'=\begin{pmatrix}a_1'\\a_2'\\\vdots\end{pmatrix}$。
...
记 $S_{\beta k}=\langle\beta|k\rangle$，$S=\begin{pmatrix}S_{11}&S_{12}&\cdots&S_{1k}&\cdots\\S_{21}&S_{22}&\cdots&S_{2k}&\cdots\\\vdots&\vdots&&\vdots&\\S_{\beta1}&S_{\beta2}&\cdots&S_{\beta k}&\cdots\end{pmatrix}$。

则 $a_\beta=\sum_k a_k\langle\beta|k\rangle$，$a'=Sa$，即 $a'_\beta=\sum_k S_{\beta k}a_k$。

$S^\dagger S=SS^\dagger=1$，即 $S^\dagger=S^{-1}$。

**注**：$a$、$a'$ 均为系数列向量，与本征态形式相似但意义完全不同。$|k\rangle$ 在希尔伯特空间中可表示为一个列向量，$|\psi\rangle$ 按 $|k\rangle$ 展开所得系数列为 $a$。

从矢量的角度考虑，$|\psi\rangle=(e_1,e_2,\dots,e_k,\dots)\begin{pmatrix}a_1\\a_2\\\vdots\end{pmatrix}$，即 $|\psi\rangle$ 是列向量 $e_i'$ 的线性组合，组合系数为列向量 $a$。

要"消去" $(e_1',e_2',\dots,e_k',\dots)$，则 $(e_1,e_2,\dots,e_k,\dots)=(e_1',e_2',\dots,e_k',\dots)\begin{pmatrix}S_{11}&S_{12}&\cdots&S_{1k}&\cdots\\S_{21}&S_{22}&\cdots&S_{2k}&\cdots\\\vdots&\vdots&&\vdots&\\S_{k1}&S_{k2}&\cdots&S_{kk}&\cdots\end{pmatrix}$。
...
$(e_1,e_2,\dots,e_k,\dots)=(e_1',e_2',\dots,e_k',\dots)\begin{pmatrix}(e_1',e_1)&(e_1',e_2)&\cdots\\(e_2',e_1)&(e_2',e_2)&\cdots\\\vdots&\vdots&\end{pmatrix}=(e_1',e_2',\dots)S$。
