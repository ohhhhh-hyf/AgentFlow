...
$$\overline{\hat{L}_x^2} = \langle l m | \hat{L}_x^2 | l m \rangle = \langle l m | (\hat{L}^2 - \hat{L}_z^2 - \hat{L}_y^2) | l m \rangle$$

由对称性 $\overline{\hat{L}_x^2} = \overline{\hat{L}_y^2}$，且 $\overline{\hat{L}_x^2} + \overline{\hat{L}_y^2} + \overline{\hat{L}_z^2} = \overline{\hat{L}^2}$，故：
...
$$\langle \phi | \phi \rangle = \langle \Psi | (\xi \hat{x} - i\hat{y})(\xi \hat{x} + i\hat{y}) | \Psi \rangle = (\xi \langle \Psi | \hat{x} - i \langle \Psi | \hat{y})(\xi \hat{x} | \Psi \rangle + i \hat{y} | \Psi \rangle)$$

$\hat{x}, \hat{y}$ 为厄米算符（$\because \hat{A}, \hat{B}$ 为厄米算符）：

$$= (\xi \langle \Psi | \hat{x}^\dagger - i \langle \Psi | \hat{y}^\dagger)(\xi \hat{x} | \Psi \rangle + i \hat{y} | \Psi \rangle)$$

$$= \xi^2 \langle \Psi | \hat{x}^2 | \Psi \rangle + \langle \Psi | \hat{y}^2 | \Psi \rangle + i\xi \left( \langle \Psi | \hat{x}\hat{y} | \Psi \rangle - \langle \Psi | \hat{y}\hat{x} | \Psi \rangle \right) = \xi^2 \langle \hat{x}^2 \rangle + \langle \hat{y}^2 \rangle + i\xi \langle [\hat{x}, \hat{y}] \rangle$$

由 $\langle \phi | \phi \rangle \ge 0$，得 $\langle \hat{x}^2 \rangle \cdot \langle \hat{y}^2 \rangle \ge \frac{1}{4} |\langle [\hat{x}, \hat{y}] \rangle|^2$，即 $\langle \hat{x}^2 \rangle \cdot \langle \hat{y}^2 \rangle \ge \frac{1}{4} |\langle [\hat{A}, \hat{B}] \rangle|^2$。

$$\therefore \Delta A \Delta B \ge \frac{1}{2} |\langle [\hat{A}, \hat{B}] \rangle|$$

**对任意力学量 $A, B$ 和任意量子态 $|\Psi\rangle$，若 $A$ 与 $B$ 不对易（即 $[\hat{A}, \hat{B}] \neq 0$），则 $\Delta A$ 和 $\Delta B$ 不能同时为零，也即 $A$ 与 $B$ 不能同时被精确测定。**
...
**定理**：哈密顿算符 $H$ 为厄米算符，满足本征方程 $H|k\rangle = E_k|k\rangle$，对体系的任一归一化态 $|\phi\rangle$，若 $H = \langle \phi|H|\phi\rangle$ 有下界（总大于某常数）但无上界，则 $H$ 的本征态 $|k\rangle$ 的集合构成体系的一个**完备集**，即体系的任一量子态 $|\phi\rangle$ 可用 $|k\rangle$ 来展开。
...
$\frac{d\bar{A}}{dt} = \frac{d}{dt}\left(\langle\psi|\hat{A}|\psi\rangle\right) = \left\langle\psi\left|\frac{\partial \hat{A}}{\partial t}\right|\psi\right\rangle + \left\langle\psi\left|\hat{A}\frac{\partial}{\partial t}\right|\psi\right\rangle + \left\langle\frac{\partial}{\partial t}\psi\left|\hat{A}\right|\psi\right\rangle = \frac{1}{i\hbar}\langle\psi|[\hat{A},\hat{H}]|\psi\rangle + \left\langle\psi\left|\frac{\partial \hat{A}}{\partial t}\right|\psi\right\rangle$

$\frac{d\bar{A}}{dt} = \frac{1}{i\hbar}\langle\psi|[\hat{A},\hat{H}]|\psi\rangle + \left\langle\psi\left|\frac{\partial \hat{A}}{\partial t}\right|\psi\right\rangle$，若 $\frac{\partial \hat{A}}{\partial t} = 0$（$\hat{A}$ 不显含 $t$）且 $[\hat{A},\hat{H}] = 0$，则 $\frac{d\bar{A}}{dt} = 0$，$\bar{A}$ 与时间无关。
...
设 $F=(A_1,A_2,\dots,A_n)$ 是一组力学量完全集，$|k\rangle$ 是它们的共同本征态，其中 $k$ 表征所有量子数。
...
$(e_1,e_2,\dots,e_k,\dots)=(e_1',e_2',\dots,e_k',\dots)\begin{pmatrix}(e_1',e_1)&(e_1',e_2)&\dots\\(e_2',e_1)&(e_2',e_2)&\dots\\\vdots&\vdots&\end{pmatrix}=(e_1',e_2',\dots)S$。
