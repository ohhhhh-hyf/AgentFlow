...
$$\bar{l}_x = \langle lm|l_x|lm\rangle = \frac{1}{i\hbar}\langle lm|[l_y, l_z]|lm\rangle = \frac{1}{i\hbar}\left[\langle lm|l_y l_z|lm\rangle - \langle lm|l_z l_y|lm\rangle\right]$$

由 $l_z|lm\rangle = m\hbar|lm\rangle$，$m$ 为实数，则 $\langle lm|l_z = m\hbar\langle lm|$  
∴ $\bar{l}_x = \frac{1}{i\hbar}\left[m\hbar\langle lm|l_y|lm\rangle - m\hbar\langle lm|l_y|lm\rangle\right] = 0$

$$\bar{l}_x = m\langle lm|l_y|lm\rangle - m\langle lm|l_y|lm\rangle = 0$$

**例2.** $|lm\rangle$ 为 $l^2$、$l_z$ 的共同本征态，求 $\bar{l}_x^2$、$\bar{l}_y^2$。

$$\bar{l}_x^2 = \langle lm|l_x^2|lm\rangle = \langle lm|(l^2 - l_z^2)|lm\rangle = l(l+1)\hbar^2 - m^2\hbar^2$$

$$\bar{l}_y^2 = \langle lm|l_y^2|lm\rangle = \langle lm|(l^2 - l_z^2)|lm\rangle = l(l+1)\hbar^2 - m^2\hbar^2$$

利用 $l_x^2 = l^2 - l_z^2$，$[l_x, l_y] = i\hbar l_z$，$l_y^2 = l^2 - l_z^2$ 等关系：

$$l_y l_z l_x - l_z = (l^2 - l_y^2) + l l_x - l_x = [l_y, l_z] + 2l_y - l_y^2$$

$$l^2 = l_x^2 + l_y^2 + l_z^2 = l(l+1)\hbar^2 + m\hbar(-m\hbar) = l(l+1)\hbar^2 - m^2\hbar^2$$

设 $\bar{l}_x^2 = \bar{l}_y^2 = \frac{1}{2}(l^2 - l_z^2) = \frac{1}{2}\left[l(l+1)\hbar^2 - m^2\hbar^2\right]$
...
考虑 $|\phi\rangle = (\hat{A} + i\lambda \hat{B})|\psi\rangle$，$\langle \phi|\phi \rangle \ge 0$，$\langle (\hat{A} + i\lambda \hat{B})\psi|(\hat{A} + i\lambda \hat{B})\psi \rangle = (\langle \psi|\hat{A} + (-i\lambda)\langle \psi|\hat{B})(\hat{A}|\psi\rangle + i\lambda \hat{B}|\psi\rangle)$

$= (\langle \psi|\hat{A} - i\lambda\langle \psi|\hat{B})(\hat{A}|\psi\rangle + i\lambda \hat{B}|\psi\rangle)$，$\hat{A}, \hat{B}$ 为厄米算符
...
**证明：** $[\hat{A}, \hat{B}] = 0 \Rightarrow \hat{A}\hat{B} = \hat{B}\hat{A}$，$\hat{B}\hat{A}|k\rangle = \hat{B} \cdot a_k|k\rangle = a_k \hat{B}|k\rangle$。则 $\hat{A}(\hat{B}|k\rangle) = a_k(\hat{B}|k\rangle)$，$\therefore \hat{B}|k\rangle$ 也是 $\hat{A}$ 属于本征值 $a_k$ 的本征态，故 $\hat{B}|k\rangle$ 与 $|k\rangle$ 是一个量子态，即 $\hat{B}|k\rangle = b_k|k\rangle$。$\hat{A}$、$\hat{B}$ 拥有共同本征态 $|k\rangle$。

**例：** $\psi(\vec{r}) = \frac{1}{(2\pi\hbar)^{3/2}} e^{i\vec{p}\cdot\vec{r}/\hbar}$ 为 $\hat{p}_x$，$\hat{p}_y$，$\hat{p}_z$ 的共同本征函数，本征值为 $p_x$，$p_y$，$p_z$。$\hat{p}\psi(\vec{r}) = -i\hbar \nabla \psi = \vec{p}\psi$。
...
**定理**：哈密顿算符 $\hat{H}$ 为厄米算符，满足本征方程 $\hat{H}|k\rangle = E_k |k\rangle$。对体系的任一归一化态 $|\psi\rangle$，若 $\langle \psi | \hat{H} | \psi \rangle$ 有下界（总大于某常数）但无上界，则 $\hat{H}$ 的本征态 $|k\rangle$ 的集合构成体系的一个**完备集**，即体系的任一量子态 $|\psi\rangle$ 可用 $|k\rangle$ 来展开。
...
$$B_n = \left[a - \frac{\sin(2a)}{2} + \frac{\sin^2(k_n a)}{k_n}\right]$$

$$B_n e^{\cdots}, \quad x > a$$
...
其中 $-kx = m\ddot{x}$，即 $\ddot{x} + \omega^2 x = 0$，$\omega^2 = \frac{k}{\mu}$。
...
$$\alpha = \sqrt{\frac{\mu\omega}{\hbar}}, \quad s = \alpha x, \quad \psi(x) = \phi(s)$$
...
要求 $\phi(s)$ 在无穷远处趋于 0（束缚态条件），**仅当 $\lambda = 2n+1$ 时，$H_n(s)$ 才有满足条件的解**，此时 $H(s) = H_n(s)$ 为厄米多项式。
...
若级数无穷，则 $H(s)$ 按 $e^{s^2}$ 量级增长，不可积。**因此级数必须只有有限项**，即存在 $n$ 使得 $a_n \neq 0$ 而 $a_{n+2} = 0$，由此得：

$$\lambda = 2n + 1$$

从而：

$$a_{j+2} = \frac{2j + 2 - 2n}{(j+2)(j+1)} a_j$$

多项式 $H(s)$ 只能含奇数项或偶数项，系数由高次项递推至低次项。

#### ② 母函数方法

方程可化为：

$$H_n''(s) - 2sH_n'(s) + 2nH_n(s) = 0$$
...
即 $\hat{A}$ 在任何态 $|\psi(t)\rangle$ 下的平均值 $\bar{A}$ 都不随时间改变。
...
证明：设 $\hat{H}$ 有共同本征函数 $\psi$，$\hat{F}\psi = F\psi$，$\hat{H}\psi = E\psi$。∵ $[\hat{G}, \hat{H}] = 0$，则 $\hat{H}(\hat{G}\psi) = \hat{G}(\hat{H}\psi) = \hat{G}(E\psi) = E(\hat{G}\psi)$。
...
$$\begin{pmatrix} \langle e'_1|e'_1\rangle & \langle e'_1|e'_2\rangle & \cdots \\ \langle e'_2|e'_1\rangle & \langle e'_2|e'_2\rangle & \cdots \\ \vdots & \vdots & \ddots \end{pmatrix} = (e'_1,e'_2,\dots,e'_k,\dots)S^\dagger$$
...
为使 $\Theta(\theta)$ 在区间 $[0,\pi]$ 上有限，**$\lambda$ 只能取 $\lambda = l(l+1)$**，其中 $l = 0,1,2,\dots$。
...
**注意**：$V(r)<0$，$R_l(r)\to 0$，$r\to\infty$ 时薛定谔方程约为 $\frac{d^2u}{dr^2}+Eu=0$，若 $E>0$，$u(r)$ 呈振荡形式，不满足束缚态条件，则 $E<0$。从能量角度分析，$E=V+K$，$K<V$，$E<0$。核与电子构成“双星模型”。
