...
于是有 $k\cot ka = -\beta$，$\beta^2 = \frac{2m(V_0 - E)}{\hbar^2}$，$E = V_0\sin^2 ka = \frac{V_0}{2}[1 - \cos(2ka)]$。
...
$$\because \overline{v_x^2} = \frac{1}{3}\overline{v^2}, \quad \therefore P = \frac{1}{3}nm\overline{v^2} = \frac{2}{3}n\overline{\varepsilon_k}$$

$\varepsilon_k = \frac{1}{2}m\overline{v^2}$ 为粒子的平均动能。

$$\because P = \frac{2}{3}n\overline{\varepsilon_k} = nkT \Rightarrow \overline{\varepsilon_k} = \frac{3}{2}kT$$
...
$$p_\theta = \mu r^2\dot{\theta} = I\dot{\theta}, \quad p_\varphi = \mu r^2\sin^2\theta\dot{\varphi} = I\sin^2\theta\dot{\varphi}$$
...
$$\Omega = \prod_j \frac{\Delta w_j^{a_j}}{a_j!} = \prod_j \frac{(\Delta\omega_j / h_0^r)^{a_j}}{a_j!}$$
...
$$\Omega = \prod_j \frac{\Delta w_j^{a_j}}{a_j!}, \quad \ln\Omega = \ln N! - \sum_j \ln a_j! + \sum_j a_j \ln(\Delta w_j)$$

$$\delta\ln\Omega = \delta\ln N! - \sum_j \delta\ln a_j! + \sum_j \ln(\Delta w_j) \cdot \delta a_j = \sum_j [\ln(\Delta w_j) - \ln a_j!]\delta a_j = 0$$
...
$$W^* = \binom{2n}{n}, \quad W = \binom{2n}{n + l} = \binom{2n}{n - l}$$

$$\ln\left(\frac{W}{W^*}\right) = \ln\frac{(n!)^2}{(n+l)!(n-l)!} \approx -\frac{2l^2}{n}$$
...
三维平动子能级 $E_{n_x n_y n_z} = \frac{\hbar^2}{2m}\left(\frac{\pi^2}{L^2}\right)(n_x^2 + n_y^2 + n_z^2)$。

刚性转子能级 $E_l = \frac{\hbar^2}{2I}l(l+1)$，$[l = 0, 1, 2, \ldots]$。
...
三维平动子：$E = \frac{1}{2m}(p_x^2 + p_y^2 + p_z^2)$，$0 \sim E$ 范围内的总量子态数 $\Sigma(E) = \frac{4\pi}{3}\frac{(2mE)^{3/2}}{h^3}V = \frac{4\pi V}{3h^3}(2mE)^{3/2}$。

双原子分子刚性转子：$E = \frac{1}{2I}\left(p_\theta^2 + \frac{p_\varphi^2}{\sin^2\theta}\right)$，$I = \mu r^2$。

令 $p_\theta = \sqrt{2IE}\cos\gamma$，$p_\varphi = \sqrt{2IE}\sin\theta \cdot \sin\gamma$，$dp_\theta dp_\varphi = \left|\begin{matrix} -\sqrt{2IE}\sin\gamma & 0 \\ \sqrt{2IE}\sin\theta\cos\gamma & \sqrt{2IE}\sin\theta\sin\gamma \end{matrix}\right| d\theta d\gamma = 2IE\sin\theta\, d\theta\, d\gamma$。

$$\Sigma(E) = \frac{1}{h^2}\int dp_\theta \int \sin\theta\, d\theta \int d\gamma \cdot I \int d\varphi = \frac{8\pi^2 IE}{h^2}$$
...
$|\varphi\rangle = \hat{A}|\psi\rangle$，在右矢空间中，$|\varphi\rangle = \hat{A}|\psi\rangle = |\hat{A}\psi\rangle$，默认 $\hat{A}$ 向右作用；在左矢空间中，$\langle\varphi| = \langle\psi|\hat{A}^\dagger$，算符 $\hat{A}^\dagger(\hat{A}^+)$ 向左作用。$\langle\psi|\hat{A}^T = \langle\hat{A}\psi| = \langle\psi|\hat{A}$。若 $\hat{A}$ 为厄米算符，则 $\langle\psi|\hat{A}^\dagger = \langle\psi|\hat{A}$。
...
$\langle\hat{A}\hat{B}\psi| = \langle\psi|\hat{B}^\dagger\hat{A}^\dagger$（先将 $\hat{B}\psi$ 视为一体，再作变换）。
...
$$\hat{\vec{L}} = \hat{\vec{r}} \times \hat{\vec{p}}, \quad \hat{\vec{L}} = \hat{\vec{r}} \times \hat{\vec{p}} = \begin{vmatrix} \hat{i} & \hat{j} & \hat{k} \\ x & y & z \\ \hat{p}_x & \hat{p}_y & \hat{p}_z \end{vmatrix}$$
...
$[\hat{L}_x, x] = [y\hat{p}_z - z\hat{p}_y, x] = 0$，$[\hat{L}_\alpha, x] = 0 \quad (\alpha = x, y, z)$。

$[\hat{L}_x, y] = [y\hat{p}_z, y] - [z\hat{p}_y, y] = -[z\hat{p}_y, y] = i\hbar z$。

$[\hat{L}_x, z] = [y\hat{p}_z - z\hat{p}_y, z] = [y\hat{p}_z, z] = -i\hbar y$（或 $\hat{L}_x z = -i\hbar y$）。

同理有 $[\hat{L}_y, x] = -i\hbar z$，$[\hat{L}_y, z] = i\hbar x$，$[\hat{L}_z, x] = i\hbar y$，$[\hat{L}_z, y] = -i\hbar x$。

$[\hat{L}_x, \hat{p}_x] = [y\hat{p}_z - z\hat{p}_y, \hat{p}_x] = 0$，$[\hat{L}_\alpha, \hat{p}_\alpha] = 0 \quad (\alpha = x, y, z)$。

$[\hat{L}_x, \hat{p}_y] = [y\hat{p}_z - z\hat{p}_y, \hat{p}_y] = [y\hat{p}_z, \hat{p}_y] = [y \cdot (-i\hbar\frac{\partial}{\partial y}), \hat{p}_y] = i\hbar\hat{p}_z$。
...
令 $\hat{L}^2 = \hat{L}_x^2 + \hat{L}_y^2 + \hat{L}_z^2$，$\hat{L}_x^2 = y^2\hat{p}_z^2 + z^2\hat{p}_y^2 - y\hat{p}_z(z\hat{p}_y) - z\hat{p}_y(y\hat{p}_z)$。

则 $\hat{L}^2 = (y^2 + z^2)\hat{p}_x^2 + (z^2 + x^2)\hat{p}_y^2 + (x^2 + y^2)\hat{p}_z^2 - (yz\hat{p}_z\hat{p}_y - y\hat{p}_z) - (yz\hat{p}_y\hat{p}_z - \hat{p}_y z\hat{p}_z) - (zx\hat{p}_x\hat{p}_z - \hat{p}_x z\hat{p}_z) - (zx\hat{p}_z\hat{p}_x - \hat{p}_z x\hat{p}_x) - (xy\hat{p}_y\hat{p}_x - \hat{p}_y x\hat{p}_x) - (xy\hat{p}_x\hat{p}_y - \hat{p}_x y\hat{p}_y)$。
...
$$[\hat{A}\hat{B}, \hat{C}] = (\hat{A}\hat{B}\hat{C} - \hat{B}\hat{C}\hat{A}) + (\hat{B}\hat{C}\hat{A} - \hat{B}\hat{C}\hat{A}) = [\hat{A}, \hat{C}]\hat{B} + \hat{A}[\hat{B}, \hat{C}]$$

$$[\hat{A}, \hat{B}\hat{C}] = [\hat{A}, \hat{B}]\hat{C} + \hat{B}[\hat{A}, \hat{C}], \quad [\hat{A}\hat{B}, \hat{C}] = \hat{A}[\hat{B}, \hat{C}] + [\hat{A}, \hat{C}]\hat{B}$$

$[\hat{L}^2, \hat{L}_x] = [\hat{L}_y^2, \hat{L}_x] + [\hat{L}_z^2, \hat{L}_x] = \hat{L}_y[\hat{L}_y, \hat{L}_x] + [\hat{L}_y, \hat{L}_x]\hat{L}_y + \hat{L}_z[\hat{L}_z, \hat{L}_x] + [\hat{L}_z, \hat{L}_x]\hat{L}_z = \hat{L}_y(-i\hbar\hat{L}_z) + (-i\hbar\hat{L}_z)\hat{L}_y + \hat{L}_z(i\hbar\hat{L}_y) + (i\hbar\hat{L}_y)\hat{L}_z = 0$。同理 $[\hat{L}^2, \hat{L}_y] = [\hat{L}^2, \hat{L}_z] = 0$。