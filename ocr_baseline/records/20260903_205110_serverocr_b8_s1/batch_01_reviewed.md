...
**思路1**：$\int_{a-\epsilon}^{a+\epsilon} \psi''(x)dx = \psi'(a+\epsilon) - \psi'(a-\epsilon) = \int_{a-\epsilon}^{a+\epsilon} [V(x) - E]\psi(x)dx$，$|\psi(x)| < \infty$，$\epsilon \to 0$ 时右式 $\to 0$
...
于是有 $k\cot ka = -\beta$，即 $\cot ka = -\frac{\beta}{k} = -\sqrt{\frac{V_0 - E}{E}}$，$E = V_0 \sin^2 ka = \frac{V_0}{2}[1 - \cos(2ka)]$
...
只考虑分子平动动能，$U = \sum_i \varepsilon_{ki} = N\overline{\varepsilon_k} = \frac{3}{2}NkT = \frac{3}{2}\nu RT$
...
即 $T_{\text{转}} = \frac{1}{2}\mu r^2(\dot{\theta}^2 + \sin^2\theta\dot{\varphi}^2) = \frac{1}{2}I(\dot{\theta}^2 + \sin^2\theta\dot{\varphi}^2)$
...
$\omega = 2\pi\nu$ 时，$\Delta\omega = \frac{E}{\nu} = h$

**偏离最概然分布的概率很小**，平衡态对应的微观态数为 $W^*$，最概然分布附近一个分布对应的微观态数为 $W$。假设 $N = 2n$ 个粒子处在一个体积 $V$ 的空间中，将 $V$ 等体积划分为 $\Delta\omega_1, \Delta\omega_2$

$$W^* = \binom{2n}{n}, \quad W = \binom{2n}{n + l}\binom{2n}{n - l}, \quad \ln\left(\frac{W}{W^*}\right) = -2n\ln\left(\frac{n + l}{n}\right) - 2n\ln\left(\frac{n - l}{n}\right) \approx -\frac{2l^2}{n}$$
...
刚性转子能级 $E_l = \frac{\hbar^2}{2I}l(l+1)$
...
**双原子分子刚性转子**：$E = \frac{1}{2I}\left(p_\theta^2 + \frac{p_\varphi^2}{\sin^2\theta}\right)$

令 $p_\theta = \sqrt{2IE}\cos\gamma$，$p_\varphi = \sqrt{2IE}\sin\theta \cdot \sin\gamma$

$$dp_\theta dp_\varphi = \begin{vmatrix} -\sqrt{2IE}\sin\gamma & 0 \\ \sqrt{2IE}\sin\theta\cos\gamma & \sqrt{2IE}\sin\theta\sin\gamma \end{vmatrix} d\theta d\gamma = 2IE\sin\theta\,d\theta\,d\gamma$$

$$\Sigma(E) = \frac{1}{h^2}\int dp_\theta dp_\varphi = \frac{2IE}{h^2}\int_0^\pi \sin\theta\,d\theta \int_0^{2\pi} d\gamma = \frac{8\pi^2 IE}{h^2}$$

（四维：$\theta, \varphi, p_\theta, p_\varphi$）
...
内积：$(\psi, \varphi) = \langle\psi|\varphi\rangle$，坐标表象 $\int \psi^*(\vec{r})\varphi(\vec{r})d\tau$，$d\tau = dxdydz$ 为微体积元，$\psi(\vec{r}) = \psi(x, y, z)$
...
$\hat{A}|\psi\rangle = |A\psi\rangle$，在右矢空间中，$|\psi'\rangle = \hat{A}|\psi\rangle = |A\psi\rangle$，默认 $\hat{A}$ 向右作用；在左矢空间中，$\langle\psi'| = \langle\psi|\hat{A}^\dagger$，算符 $\hat{A}^\dagger (\hat{A}^+)$ 向左作用。$\langle\psi|\hat{A}^T = \langle A\psi| = \langle\psi|\hat{A}$。若 $\hat{A}$ 为厄米算符，则 $\langle\psi|\hat{A} = \langle A\psi|$

$\Rightarrow$ **厄米算符在左、右矢空间中的运算具有形式不变性**

算符在左矢和右矢之间的转换本质上是对偶空间的伴随映射。

关于 $(\langle\psi|\hat{A})^\dagger = \hat{A}^\dagger|\psi\rangle$：$\langle\psi|\hat{A}|\psi\rangle = \langle\psi|\hat{A}^\dagger|\psi\rangle$，$\langle\psi|\hat{A}|\psi\rangle^* = \langle\psi|\hat{A}^\dagger|\psi\rangle \Rightarrow \langle A\psi| = \langle\psi|\hat{A}^\dagger$
...
$$\hat{\vec{L}} = \hat{\vec{r}} \times \hat{\vec{p}}, \quad \hat{L}_x = \hat{y}\hat{p}_z - \hat{z}\hat{p}_y = -i\hbar\left(y\frac{\partial}{\partial z} - z\frac{\partial}{\partial y}\right)$$
...
则 $\hat{L}^2 = (y^2 + z^2)\hat{p}_x^2 + (z^2 + x^2)\hat{p}_y^2 + (x^2 + y^2)\hat{p}_z^2 - (yz\hat{p}_y\hat{p}_z - yz\hat{p}_z\hat{p}_y) - (xz\hat{p}_x\hat{p}_z - xz\hat{p}_z\hat{p}_x) - (xy\hat{p}_x\hat{p}_y - xy\hat{p}_y\hat{p}_x)$
...
$\hat{L}^2 = (\hat{L}_x + i\hat{L}_y)(\hat{L}_x - i\hat{L}_y) + \hat{L}_z^2 - \hbar \hat{L}_z$  
（或等价地，$\hat{L}^2 = \hat{L}_- \hat{L}_+ + \hat{L}_z^2 + \hbar \hat{L}_z$，其中 $\hat{L}_\pm = \hat{L}_x \pm i\hat{L}_y$）

展开得：  
$\hat{L}^2 = \hat{L}_x^2 + \hat{L}_y^2 + \hat{L}_z^2 + i(\hat{L}_y\hat{L}_x - \hat{L}_x\hat{L}_y) - \hbar \hat{L}_z$  
利用对易关系 $[\hat{L}_x, \hat{L}_y] = i\hbar \hat{L}_z$，有 $i(\hat{L}_y\hat{L}_x - \hat{L}_x\hat{L}_y) = i(-i\hbar \hat{L}_z) = \hbar \hat{L}_z$，  
故 $\hat{L}^2 = \hat{L}_x^2 + \hat{L}_y^2 + \hat{L}_z^2$，与定义一致。

验证对易子：  
$[\hat{L}^2, \hat{L}_x] = [\hat{L}_y^2, \hat{L}_x] + [\hat{L}_z^2, \hat{L}_x]$  
