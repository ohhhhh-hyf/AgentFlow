...
由概率连续性可知，当 $E \to 0$ 时，右式趋于零，即  
$$|\psi(x)| < \epsilon \quad (\epsilon \to 0) \Rightarrow \text{右式} \to 0.$$
...
于是有：
$$\tan ka=-\frac{k}{\beta},\qquad \beta=\frac{\sqrt{2\mu(V_0-E)}}{\hbar}$$

且 $\cot ka<0$。能量满足超越方程：
$$E=\frac{\hbar^2}{2\mu a^2}\left[1-\cos(2ka)\right],\qquad \cot(ka)<0$$
...
其中 $\overline{v^2}=\frac{3kT}{m}$，$v_{rms}=\sqrt{\overline{v^2}}=\sqrt{\frac{3kT}{m}}$。

$\bar{\varepsilon}_t=\frac{1}{2}m\overline{v^2}$ 为粒子的平均平动动能。
...
定体热容：
$$C_V=\left(\frac{\partial U}{\partial T}\right)_V=\frac{3}{2}R,\qquad C_{V,m}=\frac{3}{2}R$$
...
其中 $x=r\sin\theta\cos\phi$，$y=r\sin\theta\sin\phi$，$z=r\cos\theta$。

$$E=\frac{1}{2}m(r^2\dot{\theta}^2+r^2\sin^2\theta\dot{\phi}^2)$$

对于双原子分子，两质点绕系统质心转动可约化为质量 $m_r$ 的单体转动问题（$m_r$ 为约化质量）。

系统转动能量：
$$T=\frac{1}{2}m_1 r_1^2(\dot{\theta}^2+\sin^2\theta\dot{\phi}^2)+\frac{1}{2}m_2 r_2^2(\dot{\theta}^2+\sin^2\theta\dot{\phi}^2)$$

即：
$$T=\frac{1}{2}m_r r^2(\dot{\theta}^2+\sin^2\theta\dot{\phi}^2)$$

与 $\theta,\phi$ 共轭的动量：
$$P_\theta=m_r r^2\dot{\theta},\qquad P_\phi=m_r r^2\sin^2\theta\dot{\phi}$$

$$T=\frac{1}{2m_r r^2}\left(P_\theta^2+\frac{P_\phi^2}{\sin^2\theta}\right)$$
...
把 μ 空间分成许多小体元 $\Delta\mu_j$（$j=1,2,\cdots,l$），$\Delta\mu_j$ 大小适当，足够小到可近似认为代表点落在 $\Delta\mu_j$ 内的粒子运动状态相同，但也不是无穷小，要满足 $\Delta\mu_j \geq h^3$（统计需要）。

### 宏观分布与宏观态（与 μ 空间的分布 $\{a_j\}$ 对应）
...
对 $\ln\Omega$ 求变分：
$$\delta(\ln\Omega)=\delta\left(\sum_j a_j\ln\frac{\Delta\omega_j}{a_j}\right)=\sum_j(\ln\Delta\omega_j-\ln a_j)\delta a_j=0$$

又有粒子数和能量守恒条件（孤立系）：
$$\sum_j\delta a_j=0,\qquad \sum_j\varepsilon_j\delta a_j=0$$
...
$$E=\frac{P^2}{2m}+\frac{1}{2}m\omega^2 x^2,\qquad \frac{P^2}{2mE}+\frac{m\omega^2 x^2}{2E}=1$$
...
偏离最概然分布的概率很小，平衡态对应的微观态数为 $W_m$，最概然分布附近一个分布对应的微观态数为 $W$。

假设 $N=2n$ 个粒子处在一个体积 $V$ 的空间中，将 $V$ 等体积划分为 $\Delta\mu_1,\Delta\mu_2$：

$$W=\frac{(2n)!}{(n+\Delta n)!(n-\Delta n)!}$$

$$\frac{W}{W_m}=\frac{(n!)^2}{(n+\Delta n)!(n-\Delta n)!}=\left(\frac{n}{n+\Delta n}\right)^n\left(\frac{n}{n-\Delta n}\right)^n$$
...
对于一个自由度为 $r$ 的粒子，其 $\mu$ 空间中大小为 $h$ 的相体积对应一个量子态。在动量空间中，考虑能量小于 $E$ 的态，其量子态数为  
$$
\Omega(E) = \frac{1}{h^3} \int_{H \le E} d^3q \, d^3p = \frac{V}{h^3} \cdot \frac{4\pi}{3} (2mE)^{3/2},
$$
其中 $V = abc$ 为体积。若粒子在半径为 $r$ 的球内运动，则 $V = \frac{4\pi}{3} r^3$，代入得  
$$
\Omega(E) = \frac{4\pi}{3} \frac{V}{h^3} (2mE)^{3/2} = \frac{4\pi}{3} \frac{4\pi r^3}{3 h^3} (2mE)^{3/2} = \frac{16\pi^2 r^3}{9 h^3} (2mE)^{3/2}.
$$
在球坐标下，动量空间的体积元为 $d^3p = p^2 \sin\theta \, dp \, d\theta \, d\phi$，且 $p = \sqrt{2mE}$，故 $dp = \frac{m}{p} dE$，于是  
$$
\Omega(E) = \frac{V}{h^3} \int_0^{\sqrt{2mE}} p^2 dp \int_0^\pi \sin\theta \, d\theta \int_0^{2\pi} d\phi = \frac{V}{h^3} \cdot \frac{4\pi}{3} (2mE)^{3/2}.
$$
这与上述结果一致。

对于一个自由度为 $r$ 的粒子，它的 μ 空间中大小为 $h^r$ 的相体积对应一个量子态。

三维平动子（边长为 $a,b,c$ 的方盒），第一象限 $0\sim p$ 范围内的总量子态数：

$$\Omega(p)=\frac{1}{h^3}\cdot\frac{4\pi p^3}{3}\cdot abc=\frac{4\pi V}{3h^3}p^3$$
...
坐标表象：
$$\langle\varphi|\psi\rangle=\int_{-\infty}^{\infty}\varphi^*(r)\psi(r)d\tau$$
...
$\hat{A}|\psi\rangle=|\hat{A}\psi\rangle$，在右矢空间中，$|\psi\rangle=|\psi\rangle=|\hat{A}\psi\rangle$，默认 $\hat{A}$ 向右作用。

在左矢空间中，$\langle\psi|=\langle\psi|\hat{A}^\dagger$，算符 $\hat{A}^\dagger$（$\hat{A}$ 的厄米共轭）向左作用。

$\langle\psi|\hat{A}^\dagger=\langle\hat{A}\psi|=\langle\psi|\hat{A}$。若 $\hat{A}$ 为厄米算符，则 $\langle\psi|\hat{A}=\langle\psi|\hat{A}$。
...
$\langle\psi|\hat{A}|\varphi\rangle=\langle\psi|\hat{A}\varphi\rangle$，$\langle\psi|\hat{A}|\varphi\rangle=\langle\hat{A}^\dagger\psi|\varphi\rangle$。

$$\langle\psi|\hat{A}\hat{B}|\varphi\rangle=\langle\psi|\hat{B}^\dagger\hat{A}^\dagger|\varphi\rangle$$

（先将 $\hat{B}|\varphi\rangle$ 视为一体，再作变换）。
...
（右手螺旋关系：$\hat{x}\times\hat{y}=\hat{z}$）

$$[\hat{l}_x,\hat{z}]=[\hat{y}\hat{p}_z-\hat{z}\hat{p}_y,\hat{z}]=[\hat{y}\hat{p}_z,\hat{z}]=-i\hbar\hat{y}$$

同理有：
$$[\hat{l}_y,\hat{x}]=-i\hbar\hat{z},\qquad [\hat{l}_y,\hat{z}]=i\hbar\hat{x},\qquad [\hat{l}_z,\hat{x}]=i\hbar\hat{y},\qquad [\hat{l}_z,\hat{y}]=-i\hbar\hat{x}$$

### 角动量与动量的对易式

$$[\hat{l}_x,\hat{p}_x]=[\hat{y}\hat{p}_z-\hat{z}\hat{p}_y,\hat{p}_x]=0$$

$$[\hat{l}_x,\hat{p}_y]=[\hat{y}\hat{p}_z-\hat{z}\hat{p}_y,\hat{p}_y]=[\hat{y}\hat{p}_z,\hat{p}_y]=-[\hat{z}\hat{p}_y,\hat{p}_y]=i\hbar\hat{p}_z$$

$$[\hat{l}_x,\hat{p}_z]=[\hat{y}\hat{p}_z-\hat{z}\hat{p}_y,\hat{p}_z]=-[\hat{z}\hat{p}_y,\hat{p}_z]=-i\hbar\hat{p}_y$$

同理有：
$$[\hat{l}_y,\hat{p}_x]=-i\hbar\hat{p}_z,\qquad [\hat{l}_y,\hat{p}_z]=i\hbar\hat{p}_x,\qquad [\hat{l}_z,\hat{p}_x]=i\hbar\hat{p}_y,\qquad [\hat{l}_z,\hat{p}_y]=-i\hbar\hat{p}_x$$

### 角动量各分量之间的对易式

$$[\hat{l}_x,\hat{l}_x]=0,\qquad [\hat{l}_\alpha,\hat{l}_\alpha]=0\quad(\alpha=x,y,z)$$
...
$$=[\hat{y},\hat{z}\hat{p}_x]\hat{p}_z+[\hat{z},\hat{x}\hat{p}_z]\hat{p}_y=\hat{y}[\hat{p}_z,\hat{z}]\hat{p}_x+\hat{x}[\hat{z},\hat{p}_z]\hat{p}_y=-\hat{y}(-i\hbar)\hat{p}_x+\hat{x}(-i\hbar)\hat{p}_y=i\hbar(\hat{x}\hat{p}_y-\hat{y}\hat{p}_x)=i\hbar\hat{l}_z$$
...
可得：
$$[\hat{l}^2,\hat{l}_\alpha]=0\qquad(\alpha=x,y,z)$$

**即角动量平方与各分量对易，可同时确定 $\hat{l}^2$ 和 $\hat{l}_z$ 的本征值。**