...
于是有：
$$\tan ka=-\frac{k}{\beta}=-\frac{k}{\sqrt{2\mu(V_0-E)/\hbar^2}}$$
...
其中 $\overline{v^2}=\frac{3kT}{m}$，$v_{rms}=\sqrt{\frac{3kT}{m}}$。

$$\because \overline{\varepsilon_t}=\frac{1}{2}m\overline{v^2}=\frac{3}{2}kT$$
...
$$E=\frac{p^2}{2m}+\frac{1}{2}m\omega^2 x^2 \Rightarrow \frac{p^2}{2m\varepsilon}+\frac{x^2}{2\varepsilon/(m\omega^2)}=1$$
...
系统转动惯量 $I=m_1 r_1^2+m_2 r_2^2=m_r r^2$，系统转动能量：

$$T=\frac{1}{2}m_1 r_1^2(\dot{\theta}^2+\sin^2\theta\dot{\phi}^2)+\frac{1}{2}m_2 r_2^2(\dot{\theta}^2+\sin^2\theta\dot{\phi}^2)$$

即 $T=\frac{1}{2}m_r r^2(\dot{\theta}^2+\sin^2\theta\dot{\phi}^2)=\frac{1}{2}I(\dot{\theta}^2+\sin^2\theta\dot{\phi}^2)$。

与 $\theta,\phi$ 共轭的动量：
$$p_\theta=m_r r^2\dot{\theta}=p_{\theta_1}+p_{\theta_2},\quad p_\phi=m_r r^2\sin^2\theta\dot{\phi}$$

$$T=\frac{1}{2}m_r r^2\dot{\theta}^2+\frac{1}{2}m_r r^2\sin^2\theta\dot{\phi}^2=\frac{p_\theta^2}{2I}+\frac{p_\phi^2}{2I\sin^2\theta}$$
...
子相字空间小体元 $\Delta\omega_j$（$j=1,2,\cdots,l$）中粒子运动状态数为 $\Delta w_j=\frac{\Delta\omega_j}{h_0^r}$，$a_j$ 个粒子在 $\Delta w_j$ 个运动状态上分布的可能微观状态数为 $\binom{\Delta w_j+a_j-1}{a_j}$，多个粒子可以处于同一相格内。

### 一个分布的微观分布数

$$\Omega=\frac{N!}{\prod_j a_j!}\prod_j \frac{(\Delta w_j)^{a_j}}{a_j!}$$
...
$$\delta(\ln W)=\delta\left(\sum_j a_j\ln\frac{\Delta w_j}{a_j}\right)=\sum_j(\ln\Delta w_j-\ln a_j)\delta a_j=0$$

又有粒子数和能量守恒条件（孤立系）：
$$\sum_j\delta a_j=0,\quad \sum_j\varepsilon_j\delta a_j=0$$

令 $f(a_1,a_2,\cdots,a_l,\alpha,\beta)=\sum_j a_j(\ln\Delta w_j-\ln a_j)+\alpha\left(\sum_j a_j-N\right)+\beta\left(\sum_j\varepsilon_j a_j-E\right)$

则 $f$ 取极值时，$\frac{\partial f}{\partial a_j}=0$，即 $-\ln a_j+\ln\Delta w_j+\alpha+\beta\varepsilon_j=0$。

$$a_j=\Delta w_j e^{-\alpha-\beta\varepsilon_j}$$
...
一维线性谐振子 $E=\frac{p^2}{2m}+\frac{1}{2}m\omega^2 x^2$，$\frac{p^2}{2mE}+\frac{x^2}{2E/(m\omega^2)}=1$，代表点的轨迹为椭圆（μ空间）。
...
- 平动子能级间隔极小，可视为连续分布，按 $\Delta\omega$ 分能级；常温下，转动也可看作连续（不含 $H_2$）；振动一般必须看作分立能级，直接分析量子能级简并度。
...
$$\Omega(p)=\frac{1}{h^3}\cdot\frac{4\pi}{3}p^3\cdot V=\frac{4\pi V}{3h^3}(2mE)^{3/2}$$
...
**内积**：$(\phi,\psi)=\langle\phi|\psi\rangle$

坐标表象：$\langle\phi|\psi\rangle=\int_{-\infty}^{\infty}\phi^*(r)\psi(r)d\tau$，$d\tau=dxdydz$ 为微体积元，$\psi(r)=\psi(x,y,z)$。
...
关于 $(\langle\phi|\hat{A}|\psi\rangle)^*=\langle\psi|\hat{A}^\dagger|\phi\rangle$：
...
$$[\hat{l}_a,\hat{p}_a]=0\quad(a=x,y,z)$$
...
$$[\hat{l}_x,\hat{l}_x]=0,\quad[\hat{l}_a,\hat{l}_a]=0\quad(a=x,y,z)$$
...
$$=[\hat{y},\hat{z}\hat{p}_x]\hat{p}_z+[\hat{z},\hat{x}\hat{p}_z]\hat{p}_y=\hat{y}[\hat{p}_z,\hat{z}]\hat{p}_x+\hat{x}[\hat{z},\hat{p}_z]\hat{p}_y=-i\hbar\hat{y}\hat{p}_x+i\hbar\hat{x}\hat{p}_y=i\hbar\hat{l}_z$$
...
$$[\hat{l}^2,\hat{l}_a]=0\quad(a=x,y,z)$$
