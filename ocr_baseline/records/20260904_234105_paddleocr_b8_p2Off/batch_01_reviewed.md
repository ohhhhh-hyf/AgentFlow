...
在一维情况下，A4=E4简化为：[-\$数+V(x)](x)=E4(x).

时，能量本征函数4(x)及其导函数(x)={dyx在a点是连续的.
4(x)本身连续是因为概率幅的连续性问题(4(x)是概率波!)  
14(x)|<1E→0时右式→0  
思路2：反证法，若(x)不连续，则(x)含冲击顶，不符合薛定湾方程  
一维半无限深方势阱
...
- **$x<0$ 区域**：$\because V(x)=\infty$，物理上不允许粒子在此区域出现，故有 $\psi(x)=0 \quad (x<0)$。
- **边界条件**：$\psi(0)=0$，则 $\psi(x) = A\sin kx$（$0<x<a$ 区域）。
...
且 $\cot ka < 0$。能量满足超越方程：  
$$E = \frac{\hbar^2 k^2}{2\mu} = \frac{\hbar^2}{2\mu a^2}\left[1 - \cos(2ka)\right] \quad \text{且} \quad \cot(ka) < 0$$
...
$$P = \frac{2mv_{ix}}{t} \cdot \frac{N}{V} = nm\overline{v_x^2} = \frac{1}{3}nm\overline{v^2}$$

又 $\overline{v^2} = \frac{3kT}{m}$，则：

$$P = nkT$$

$E_k = \frac{1}{2}m\overline{v^2}$ 为粒子的平均动能，$\therefore P = \frac{2}{3}nE_k = nkT \Rightarrow E_k = \frac{3}{2}kT$

### 分子的平均动能与理想气体系统的内能

只考虑分子平动动能，内能：

$$U = NE_k = N \cdot \frac{3}{2}kT = \frac{3}{2}RT$$
...
由 $\sum a_j = N$ 可确定 $\alpha$，进而 $W = \frac{N!}{\prod a_j!}$。
...
设 $A$ 和 $A^\dagger$ 是两个算符，对 $|\psi\rangle$ 和 $|\varphi\rangle$，若 $\langle\varphi|A|\psi\rangle = \langle\psi|A^\dagger|\varphi\rangle^*$，则称 $A^\dagger$ 为 $A$ 的转置算符（共轭算符）。

- 在右矢空间中，$A|\psi\rangle = |A\psi\rangle$，默认 $A$ 向右作用；在左矢空间中，$\langle\psi|A = \langle A^\dagger\psi|$，算符 $A^\dagger$（$A^\dagger$）向左作用。
- $\langle\psi|A^\dagger = \langle A\psi| = \langle\psi|A$。若 $A$ 为厄米算符，则 $\langle\psi|A = \langle A\psi|$。
...
$$=x^2=x^2=1 \quad e^x x^3 2 \quad 2^2 1+2+xx^1=$$

$$z^{-2}dh=(z-h)^{-}= \times 1 \quad 2dx-yz=(x-xz)^4! -=1 \quad lz=-it(x-y=x-y)$$
...
$$[x, P_x]=[yP_z-zP_y, P_x]=0 \quad [l_a, P_a]=0 \quad \alpha=x, y, z$$
...
则 $l^2=(x^2+y^2)P_z^2+(z+x)^2+(y^2+z^2)^2-(yz^4P_2)- (yzPP_2)-(zxP_zP)$

$$-(xzxP_2-xx)-(xyPP-itxx)-(yxP-yP)$$
