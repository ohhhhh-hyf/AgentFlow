...
考虑 $|\phi\rangle = (\hat{A} + i\lambda \hat{B})|\psi\rangle$，$\langle \phi|\phi \rangle \ge 0$：

$$\langle (\hat{A} + i\lambda \hat{B})\psi | (\hat{A} + i\lambda \hat{B})\psi \rangle = (\langle \psi|\hat{A} - i\lambda \langle \psi|\hat{B})(\hat{A}|\psi\rangle + i\lambda \hat{B}|\psi\rangle)$$
...
$\because \forall \lambda$，$\langle \phi|\phi \rangle \ge 0$，$\therefore (i\langle [\hat{A},\hat{B}] \rangle)^2 \le 4\langle \hat{A}^2 \rangle \cdot \langle \hat{B}^2 \rangle$，即 $\langle \hat{A}^2 \rangle \cdot \langle \hat{B}^2 \rangle \ge \frac{1}{4}|\langle [\hat{A},\hat{B}] \rangle|^2$，$\therefore \langle x^2 \rangle \cdot \langle y^2 \rangle \ge \frac{1}{4}|\langle [x, y] \rangle|^2$。
...
$$\therefore A = A^\dagger,\ \langle A \rangle \text{ 为实数}$$

即：
$$\langle \psi | A^\dagger | \psi \rangle = \langle A \psi | \psi \rangle,\quad \langle \psi | A | \psi \rangle = \langle A^\dagger \psi | \psi \rangle,\quad A = A^\dagger = A^*,\ \langle A \rangle \text{ 为实数}$$

$$\langle \psi | A^2 | \psi \rangle = \langle \psi | A A | \psi \rangle = \langle \psi | A^\dagger A | \psi \rangle = \langle A \psi | A \psi \rangle = \langle \Phi | \Phi \rangle \ge 0,\quad \therefore \langle A^2 \rangle \ge 0$$

其中 $|\Phi\rangle = A|\psi\rangle$。
...
又 $\langle k | F | k \rangle = \langle F k | k \rangle = \lambda_k^*$，且 $\langle k | k \rangle = 1$，故 $\lambda_k = \lambda_k^*$，即 **$\lambda_k$ 为实数**。
...
若对任意 $|\psi\rangle$，有 $P_k|\psi\rangle = \sum_k |k\rangle\langle k|\psi\rangle = |\psi\rangle$，则称基矢 $|k\rangle$ 具有**完备性**（任意 $|\psi\rangle$ 可按 $|k\rangle$ 展开）。

记 $C_k = \langle k|\psi\rangle$，则：
$$|\psi\rangle = \sum_k |k\rangle\langle k|\psi\rangle = \sum_k C_k |k\rangle$$

其中 $C_k$ 为用 $|k\rangle$ 将 $|\psi\rangle$ 展开时的展开系数。
...
$$B_n = \left[a - \frac{\sin(2a)}{2}\right] + \sin^2(k_n a) \sin(k_n)$$
...
$$\alpha = \sqrt{\frac{\mu\omega}{\hbar}}, \quad s = \alpha x, \quad \psi(x) = \phi(s) = \phi(\alpha x)$$
...
要求 $\phi(s)$ 在无穷远处趋于 0（平方可积），**仅当 $\lambda = 2n + 1$ 时**，$H_n(s)$ 才有满足条件的解，此时 $H(s) = H_n(s)$ 为厄米多项式。
...
若级数无穷，则 $\phi(s)$ 按 $e^{s^2}$ 量级增长，不可积。**因此级数必须只有有限项**，即存在 $n$ 使得 $a_n \neq 0$ 而 $a_{n+2} = 0$，由此得：
...
角向方程：  
$$\frac{1}{\sin\theta}\frac{\partial}{\partial\theta}\left(\sin\theta\frac{\partial}{\partial\theta}\right)+\frac{1}{\sin^2\theta}\frac{\partial^2}{\partial\phi^2}+l(l+1)=0$$
...
从矢量的角度考虑，$|\psi\rangle = \sum_k a_k e_k$（列向量 $e_k$ 的线性组合）。要消去 $e_k$，则 $(e'_1, e'_2, \cdots) = (e_1, e_2, \cdots) S$，合系数列。
...
其中矩阵元 $S_{ik} = \langle e_i | e'_k \rangle$，即
$$\begin{pmatrix} \langle e_1|e'_1\rangle & \langle e_1|e'_2\rangle & \cdots & \langle e_1|e'_k\rangle \\ \langle e_2|e'_1\rangle & \langle e_2|e'_2\rangle & \cdots & \langle e_2|e'_k\rangle \\ \vdots & \vdots & \ddots & \vdots \\ \langle e_k|e'_1\rangle & \langle e_k|e'_2\rangle & \cdots & \langle e_k|e'_k\rangle \end{pmatrix} = S$$

## 球谐函数与角向方程

上一页末尾涉及 $e^{i\phi}$ 与 $e^{-i\phi}$ 的组合，本页继续推导角向方程的解。

由分离变量法，设 $Y(\theta,\phi)=\Theta(\theta)\Phi(\phi)$，代入角向方程：
...
$$
\int_0^\pi \Theta(\theta)\sin\theta\,d\theta = \delta_{ll'}
$$
...
其中 $P_l^m(\cos\theta)$ 为关联勒让德函数，$Y_{lm}$ 是 $L^2$ 和 $L_z$ 的共同本征函数。
...
$$\because k^2=\frac{\mu e^4}{2\hbar^2 n^2}\quad\therefore E_n=-\frac{\mu e^4}{2\hbar^2 n^2}$$
...
明德华单术美创都
Y OF SCENCEANDT HUAZHONG uNIVERSITYOF SCIENCE AND TECHNOLOGY
