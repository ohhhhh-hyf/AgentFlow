...
其中 $m$ 对应的角部分概率为 $|Y_{lm}(\theta, \varphi)|^2$。
...
$$P(\theta, \theta+d\theta; \ \varphi, \varphi+d\varphi) = \int |Y_{lm}(\theta, \varphi)|^2 \sin\theta \, d\theta \, d\varphi$$
...
$$\vec{j} = \frac{e\hbar m}{m_e r \sin\theta} |\psi|^2 \hat{e}_\varphi$$

电流为围绕 $z$ 轴的许多环形电流（在 $x$-$y$ 平面内）：

$$d\vec{I} = \vec{j} \cdot d\vec{S}$$
...
其中 $\vec{S}$ 为环形电流所围面积，$S = \pi (r\sin\theta)^2$。
...
两个面的通量之差为 $(J_b \, dv \, dw) \cdot du = \frac{\partial}{\partial u}(J_b) \, du \, dv \, dw$。

同理，另外两个通量差为 $\frac{\partial}{\partial v}(J_c) \, du \, dv \, dw$，$\frac{\partial}{\partial w}(J_d) \, du \, dv \, dw$。

$$\therefore (\nabla \cdot F) \, dV = \left( \frac{\partial}{\partial u} + \frac{\partial}{\partial v} + \frac{\partial}{\partial w} \right) J \, du \, dv \, dw = \left( \frac{\partial}{\partial u}(J_b) + \frac{\partial}{\partial v}(J_c) + \frac{\partial}{\partial w}(J_d) \right) du \, dv \, dw$$
...
$$\text{rot } \mathbf{a} = \lim_{S \to 0} \frac{\oint \mathbf{a} \cdot d\mathbf{s}}{S} = \nabla \times \mathbf{a} = \left( \frac{\partial a_w}{\partial v} - \frac{\partial a_v}{\partial w} \right) \mathbf{e}_u + \left( \frac{\partial a_u}{\partial w} - \frac{\partial a_w}{\partial u} \right) \mathbf{e}_v + \left( \frac{\partial a_v}{\partial u} - \frac{\partial a_u}{\partial v} \right) \mathbf{e}_w$$
...
$$\int_{M_2 N_2} \mathbf{a} \cdot d\mathbf{s} = a(u, v + \Delta v, w) \cdot d\mathbf{s} = a(u, v + \Delta v, w) H_w \, dw = a_w(u, v + \Delta v, w) H_w(u, v + \Delta v, w) \, dw$$

$$\int_{N_2 N_1} \mathbf{a} \cdot d\mathbf{s} = a(u, v, w + \Delta w) \cdot d\mathbf{s} = -a(u, v, w + \Delta w) H_v \, dv = -a_v(u, v, w + \Delta w) H_v(u, v, w + \Delta w) \, dv$$

$$\int_{N_1 M_1} \mathbf{a} \cdot d\mathbf{s} = -a(u, v, w) H_w \, dw = -a_w(u, v, w) H_w(u, v, w) \, dw$$

则

$$\oint \mathbf{a} \cdot d\mathbf{s} = \left[ \frac{\partial}{\partial v}(a_w H_w) - \frac{\partial}{\partial w}(a_v H_v) \right] dv \, dw = (\text{rot } \mathbf{a})_u \, dv \, dw = \frac{1}{H_v H_w} \left[ \frac{\partial}{\partial v}(a_w H_w) - \frac{\partial}{\partial w}(a_v H_v) \right] H_u H_v H_w \, dv \, dw$$
...
展开得
$$\frac{1}{2\mu}\left[p_x^2 + p_y^2 + p_z^2 + \frac{qB}{2}(x p_y - y p_x) + \frac{q^2 B^2}{4}(x^2 + y^2)\right] + V(r)$$
...
**注意**：① 左式 $= \frac{1}{r^2}\frac{d}{dr}\left(r^2\frac{dR}{dr}\right) - \frac{l(l+1)}{r^2}R + \left[\frac{2\mu}{\hbar^2}(E - V) - \frac{qB}{\hbar}m - \frac{q^2B^2}{4\hbar^2}\rho^2\right]R = 0$

$$-\frac{\hbar^2}{2\mu}\left[\frac{1}{r^2}\frac{d}{dr}\left(r^2\frac{dR}{dr}\right)\right] + \left[\frac{l(l+1)\hbar^2}{2\mu r^2} + V(r) + \frac{qB\hbar}{2\mu}m + \frac{q^2B^2}{8\mu}\rho^2 - E\right]R = 0$$

令 $E_0 = E - \omega_L m\hbar = E - \frac{qB\hbar}{2\mu}m$，代入 $V(r) = -\frac{k}{r} - \lambda k \frac{1}{r^2}$：

则
$$\frac{1}{r^2}\frac{d}{dr}\left(r^2\frac{dR}{dr}\right) - \left[\frac{l(l+1)}{r^2} - \frac{2\mu}{\hbar^2}\left(-\frac{k}{r} - \frac{\lambda k}{r^2}\right) - \frac{2\mu}{\hbar^2}(\omega_L m\hbar - E)\right]R = 0$$

$$\frac{d^2R}{dr^2} + \frac{2}{r}\frac{dR}{dr} + \left[\frac{2\mu}{\hbar^2}\left(E_0 + \frac{k}{r}\right) - \frac{l(l+1) - 2\mu\lambda k/\hbar^2}{r^2}\right]R(r) = 0$$
