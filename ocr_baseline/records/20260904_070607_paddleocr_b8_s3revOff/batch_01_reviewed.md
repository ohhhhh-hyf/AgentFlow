...
- 思路 3：连续性方程 $\frac{\partial P(x,t)}{\partial t} + \frac{\partial j(x,t)}{\partial x} = 0$，其中 $P = \psi^*\psi = |\psi|^2$，$j = -\frac{i\hbar}{2\mu}(\psi^*\nabla\psi - \psi\nabla\psi^*)$。连续 $\Rightarrow \psi, \psi'$ 连续
...
- 在 $x = a$ 处连续：$\psi(a) = Be^{-\beta a} = A\sin ka$，$\psi'(a) = -\beta Be^{-\beta a} = kA\cos ka$。
...
$$P = \frac{2mv_x}{\Delta t} \cdot \frac{N}{V} = nm\overline{v_x^2}$$

由 $\overline{v_x^2} = \frac{1}{3}\overline{v^2}$，得：

$$P = \frac{1}{3}nm\overline{v^2} = \frac{2}{3}n\bar{\varepsilon}_k$$

其中 $\bar{\varepsilon}_k = \frac{1}{2}m\overline{v^2}$ 为粒子的平均动能。

$$\therefore P = nkT \Rightarrow \bar{\varepsilon}_k = \frac{3}{2}kT$$

### 分子的平均动能与理想气体系统的内能

只考虑分子平动动能，内能：

$$U = N\bar{\varepsilon}_k = N \cdot \frac{3}{2}kT = \frac{3}{2}RT$$
...
$$\Phi(\varepsilon) = \frac{4\pi}{3}\left(\frac{2\mu\varepsilon}{h^2}\right)^{3/2} \cdot abc = \frac{4\pi V}{3h^3}(2\mu\varepsilon)^{3/2}$$

$$p_r^2 = 2\mu\varepsilon_r, \quad \sin^2\theta$$
...
由角动量定义 $\mathbf{L} = \mathbf{r} \times \mathbf{p}$，即 $l_x = y p_z - z p_y$，$l_y = z p_x - x p_z$，$l_z = x p_y - y p_x$。

#### 角动量与坐标的对易关系

$$[l_x, x] = [y p_z - z p_y, x] = 0, \quad [l_\alpha, x_\alpha] = 0 \quad (\alpha = x, y, z)$$
...
$$[l_x, p_x] = [y p_z - z p_y, p_x] = 0, \quad [l_\alpha, p_\alpha] = 0 \quad (\alpha = x, y, z)$$

$$[l_x, p_y] = [y p_z - z p_y, p_y] = [y p_z, p_y] = [y, p_y] p_z = i\hbar p_z$$
...
$$= [y, z p_x] p_z + [z p_y, x p_z] = y [p_z, z] p_x + x [z, p_z] p_y = y(-i\hbar) p_x - x(-i\hbar) p_y = i\hbar l_z$$
...
令 $l^2 = l_x^2 + l_y^2 + l_z^2 = (y p_z - z p_y)^2 + (z p_x - x p_z)^2 + (x p_y - y p_x)^2$

则：

$$l^2 = (y^2 + z^2) p_x^2 + (z^2 + x^2) p_y^2 + (x^2 + y^2) p_z^2 - (yz p_z p_y + yz p_y p_z) - (zx p_z p_x + zx p_x p_z) - (xy p_x p_y + xy p_y p_x)$$
...
由上述恒等式可证 $[l^2, l_x] = [l^2, l_y] = [l^2, l_z] = 0$，即角动量平方算符与各分量对易。
