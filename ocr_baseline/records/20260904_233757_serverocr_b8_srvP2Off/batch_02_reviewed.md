...
$$\hat{L}_x = \langle l m | \hat{L}_x | l m \rangle = \frac{1}{i\hbar} \langle l m | [\hat{L}_y, \hat{L}_z] | l m \rangle = \frac{1}{i\hbar} \left[ \langle l m | \hat{L}_y \hat{L}_z | l m \rangle - \langle l m | \hat{L}_z \hat{L}_y | l m \rangle \right]$$

由于 $\hat{L}_z |l m\rangle = m\hbar |l m\rangle$，$m$ 为实数，则 $\langle l m | \hat{L}_z = m\hbar \langle l m |$，代入得：

$$\hat{L}_x = \frac{1}{i\hbar} \left[ m\hbar \langle l m | \hat{L}_y | l m \rangle - m\hbar \langle l m | \hat{L}_y | l m \rangle \right] = \frac{1}{i\hbar} \left[ m\hbar \langle l m | \hat{L}_y | l m \rangle - m\hbar \langle l m | \hat{L}_y | l m \rangle \right] = 0$$

即 $\hat{L}_x = 0$，同理 $\hat{L}_y = 0$。

**例2.** $|l m\rangle$ 为 $(\hat{L}^2, \hat{L}_z)$ 的共同本征态，求 $\overline{\hat{L}_x^2}$，$\overline{\hat{L}_y^2}$。

$$\overline{\hat{L}_x^2} = \langle l m | \hat{L}_x^2 | l m \rangle = \langle l m | (\hat{L}^2 - \hat{L}_z^2) | l m \rangle = l(l+1)\hbar^2 - m^2\hbar^2 = \langle l m | \hat{L}_z^2 | l m \rangle = m^2\hbar^2$$
...
$$\overline{\hat{L}_x^2} = \langle l m | \hat{L}_x^2 | l m \rangle = \langle l m | (\hat{L}_y \hat{L}_x + i\hbar \hat{L}_z) | l m \rangle = \langle l m | \hat{L}_y \hat{L}_x | l m \rangle + i\hbar \langle l m | \hat{L}_z | l m \rangle$$

由于 $\hat{L}_x |l m\rangle$ 与 $|l m\rangle$ 正交（不同 $m$ 值），故 $\langle l m | \hat{L}_y \hat{L}_x | l m \rangle = 0$，且 $\langle l m | \hat{L}_z | l m \rangle = m\hbar$，因此：

$$\overline{\hat{L}_x^2} = i\hbar \cdot m\hbar = im\hbar^2$$

但 $\overline{\hat{L}_x^2}$ 应为实数，故需重新考虑。实际上由对称性：
...
$$\Delta A = \sqrt{\langle \hat{A}^2 \rangle - \langle \hat{A} \rangle^2} = \sqrt{\langle (\hat{A} - \langle A \rangle)^2 \rangle}, \quad \Delta B = \sqrt{\langle \hat{B}^2 \rangle - \langle \hat{B} \rangle^2}$$
...
令 $\hat{x} = \hat{A} - \langle A \rangle$，$\hat{y} = \hat{B} - \langle B \rangle$，要证变为：$\langle \hat{x}^2 \rangle \cdot \langle \hat{y}^2 \rangle \ge \frac{1}{4} |\langle [\hat{x}, \hat{y}] \rangle|^2$。联想 $b^2 \ge 4ac$。

考虑 $|\phi\rangle = (\xi \hat{A} + i \hat{B})|\psi\rangle$，$\langle \phi | \phi \rangle \ge 0$：

$$\langle \phi | \phi \rangle = \langle \psi | (\xi \hat{A} + i \hat{B})^\dagger (\xi \hat{A} + i \hat{B}) | \psi \rangle = \langle \psi | (\xi \hat{A}^\dagger - i \hat{B}^\dagger)(\xi \hat{A} + i \hat{B}) | \psi \rangle$$

$\hat{A}, \hat{B}$ 为厄米算符，故 $\hat{A}^\dagger = \hat{A}$，$\hat{B}^\dagger = \hat{B}$：

$$= \langle \psi | (\xi \hat{A} - i \hat{B})(\xi \hat{A} + i \hat{B}) | \psi \rangle$$

$$= \xi^2 \langle \psi | \hat{A}^2 | \psi \rangle + \langle \psi | \hat{B}^2 | \psi \rangle + i\xi \left( \langle \psi | \hat{A}\hat{B} | \psi \rangle - \langle \psi | \hat{B}\hat{A} | \psi \rangle \right) = \xi^2 \langle \hat{A}^2 \rangle + \langle \hat{B}^2 \rangle + i\xi \langle [\hat{A}, \hat{B}] \rangle$$

由 $\langle \phi | \phi \rangle \ge 0$，即 $\xi^2 \langle \hat{A}^2 \rangle + \langle \hat{B}^2 \rangle + i\xi \langle [\hat{A}, \hat{B}] \rangle \ge 0$，故判别式 $\le 0$，即 $\langle \hat{A}^2 \rangle \cdot \langle \hat{B}^2 \rangle \ge \frac{1}{4} |\langle [\hat{A}, \hat{B}] \rangle|^2$。
...
证明：$[\hat{A}, \hat{B}] = 0 \Rightarrow \hat{A}\hat{B} = \hat{B}\hat{A}$。$\hat{B}\hat{A}|k\rangle = \hat{B} \cdot a_k |k\rangle = a_k \hat{B}|k\rangle$，则 $\hat{A}(\hat{B}|k\rangle) = a_k (\hat{B}|k\rangle)$，故 $\hat{B}|k\rangle$ 也是 $A$ 属于本征值 $a_k$ 的本征态。由于 $a_k$ 不简并，$\hat{B}|k\rangle$ 与 $|k\rangle$ 是同一个量子态，即 $\hat{B}|k\rangle = b_k |k\rangle$。因此 $A$、$B$ 拥有共同本征态 $|k\rangle$。
...
**转置算符**：对 $V$ 和 $Y$，若 $\langle \phi|A|\psi\rangle = \langle \psi|A^\dagger|\phi\rangle$，则称 $A$ 和 $A^\dagger$ 互为彼此的转置算符。
...
厄米算符 $A$ 在任意量子态下的平均值 $\bar{A}$ 为实数，$A^2$ 的平均值 $\overline{A^2} \ge 0$。
...
则 $\langle k|F|k\rangle = \bar{F} = \langle k|\lambda_k k\rangle = \lambda_k \langle k|k\rangle = \lambda_k$，因 $\lambda_k = \bar{F}$ 为实数。
...
若 $\bar{H} = \langle \phi|H|\phi\rangle$ 有下界（总大于某常数）但无上界，则 $H$ 的本征态 $|k\rangle$ 的集合构成体系的一个**完备集**，即体系的任一量子态 $|\phi\rangle$ 可用 $|k\rangle$ 来展开。
...
又 \(G\psi_4 = FG\psi_4 \neq FG\psi_4\)，则 \(G\psi_4\) 不是 \(F\) 的本征态，\(G\psi_4\) 和 \(\psi_4\) 不是一个态，即 \(E\) 对应至少两个态，能级简并。

（此处缺失行均为页眉或地址残留，如“UNIVERSITY OF SCIENCE AND TECHNOLOGY”“R.China 中国·武汉”等，不属正文内容，故不补入。）

表象变换与矩阵力学

设 \(F=(A_1, A_2, \ldots, A_n)\) 是一组力学量完全集，\(\psi_k \equiv |k\rangle\) 是其共同本征态，其中 \(k\) 表征所有量子数。
...
$\bar{A}(t) = \langle \psi(t) | \hat{A} | \psi(t) \rangle$。薛定谔方程：$i\hbar \frac{\partial}{\partial t}|\psi(t)\rangle = \hat{H}|\psi(t)\rangle$。在左矢空间中：$-i\hbar \frac{\partial}{\partial t}\langle \psi | = \langle \psi | \hat{H}$（$\hat{H}^\dagger = \hat{H}$）。

$\frac{d\bar{A}}{dt} = \frac{d}{dt}\left( \langle \psi | \hat{A} | \psi \rangle \right) = \left\langle \frac{\partial \psi}{\partial t} \Big| \hat{A} \Big| \psi \right\rangle + \left\langle \psi \Big| \frac{\partial \hat{A}}{\partial t} \Big| \psi \right\rangle + \left\langle \psi \Big| \hat{A} \Big| \frac{\partial \psi}{\partial t} \right\rangle = -\frac{1}{i\hbar}\langle \psi | \hat{H}\hat{A} | \psi \rangle + \left\langle \psi \Big| \frac{\partial \hat{A}}{\partial t} \Big| \psi \right\rangle + \frac{1}{i\hbar}\langle \psi | \hat{A}\hat{H} | \psi \rangle$
...
在 $F$ 表象中，$|k\rangle=\psi_k$，$|\psi\rangle=\sum_k a_k|k\rangle$，$|\psi\rangle$ 在 $F$ 表象中可用系数列向量表示为 $a=\begin{pmatrix}a_1\\a_2\\\vdots\end{pmatrix}$。

在另一 $F'$ 表象中，$|\beta\rangle=\psi_\beta$ 为基矢，$|\psi\rangle=\sum_\beta a_\beta|\beta\rangle$，在 $F'$ 表象中可表示为 $a'=\begin{pmatrix}a_1'\\a_2'\\\vdots\end{pmatrix}$。
...
记 $S_{\beta k}=\langle\beta|k\rangle$，$S=\begin{pmatrix}S_{11}&S_{12}&\cdots\\S_{21}&S_{22}&\cdots\\\vdots&\vdots&\ddots\end{pmatrix}$。

则 $a_\beta=\sum_k a_k\langle\beta|k\rangle$，$a'=Sa$，即 $a'_\beta=\sum_k S_{\beta k}a_k$。

$S^\dagger S=SS^\dagger=1$，即 $S$ 为**么正矩阵**。

> **注**：$a$、$a'$ 均为系数列向量，与本征态形式相似但意义完全不同。$|k\rangle$ 在希尔伯特空间中可表示为一个列向量，$|\psi\rangle$ 按 $|k\rangle$ 展开所得系数列为 $a$。

从矢量的角度考虑，$|\psi\rangle=(e_1,e_2,\dots,e_k,\dots)\begin{pmatrix}a_1\\a_2\\\vdots\end{pmatrix}$，即列向量 $e_i'$ 的线性组合系数列。

要"消去" $(e_1',e_2',\dots,e_k',\dots)$，则 $(e_1,e_2,\dots,e_k,\dots)=(e_1',e_2',\dots,e_k',\dots)\begin{pmatrix}S_{11}&S_{12}&\cdots&S_{1k}&\cdots\\S_{21}&S_{22}&\cdots&S_{2k}&\cdots\\\vdots&\vdots&&\vdots\\S_{k1}&S_{k2}&\cdots&S_{kk}&\cdots\\\vdots&\vdots&&\vdots\end{pmatrix}$。
...
$(e_1,e_2,\dots)(e_1,e_2,\dots)^\dagger$ 等内积关系可统一写为 $(e_i,e_j)=\delta_{ij}$，$(e_1',e_2',\dots)=(e_1,e_2,\dots)S$，即 $(e_1',e_2',\dots)=(e_1,e_2,\dots)S$。
