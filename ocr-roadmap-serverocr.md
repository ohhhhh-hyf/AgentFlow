...
- `layout.merge_fragment_lines`：同页碎片按**保守视觉邻接**合并（垂直间隙 ≤
  ...
  编号开头、公式行不参与/作隔断）；文本中文相连不加空格、中英交界加空格；
  conf 取碎片最低值；bbox 取并集；零 token；
- 在版面推断**之前**执行 → 下游对"逻辑行"只做一次角色/标题判定；
- 开关 `OCR_MERGE_FRAGMENTS=1`（默认 0）；常量默认值为保守设定、非标定目标；
- 离线测试：`ocr_baseline/test_step5.py`（6 组用例，全绿）。

**待办（server A/B，性质验收，不预设数值）**：
- `OCR_MERGE_FRAGMENTS=1` vs 默认各跑一次 server 基线；
- 看：每页参考行数/每页 prompt 是否下降、墙钟与 token 方向、kept80/公式 avg/
  入库增量不劣化、确定性页数与触发分布是否更合理；
- 决定是否默认开启（paddle 亦可试点，机制引擎无关）。
