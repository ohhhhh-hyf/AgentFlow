...
- 思路 3（概率守恒）：$\frac{\partial P(x,t)}{\partial t} + \frac{\partial j(x,t)}{\partial x} = 0$，其中 $P = \psi^*\psi = |\psi|^2$，$j = -\frac{i\hbar}{2\mu}(\psi^*\frac{\partial \psi}{\partial x} - \psi \frac{\partial \psi^*}{\partial x})$。连续性要求 $\psi$ 与 $\psi'$ 连续。
...
3. **$x > a$ 区域**：方程化为 $\frac{d^2\psi}{dx^2} - \frac{2\mu}{\hbar^2}(V_0 - E)\psi = 0$。令 $\beta = \sqrt{\frac{2\mu(V_0 - E)}{\hbar^2}}$，则 $\psi(x) = Be^{\beta x} + Ce^{-\beta x}$。
...
$B_n = \sqrt{\frac{2}{a - \sin(2a) + \sin^2(k_n a)}}$，$\psi_n(x) = B_n e^{-\kappa x}$，$x > a$。一个 $E_n$ 对应一个 $\psi_n$。
...
$s \to \infty$ 时，$\phi(s) \sim e^{s^2}$ 量级增长，不可积。级数存在（只有）有限项：$a_n \neq 0$，$a_{n+2} = 0$，即 $\lambda = 2n + 1$。

从而有 $a_{j-2} = \frac{2j - 1 - \lambda}{j(j-1)} a_j$，多项式 $H(s)$ 只能含奇数项或偶数项，系数由高次项推至低次项。
...
**球谐函数：**
$$Y_{lm}(\theta,\phi)=\Theta_{lm}(\theta)\Phi_m(\phi)=N_{lm}P_l^m(\cos\theta)e^{im\phi}=\sqrt{\frac{(2l+1)(l-|m|)!}{4\pi(l+|m|)!}}P_l^m(\cos\theta)e^{im\phi}$$

球谐函数满足正交关系：
$$\int_0^{2\pi}\int_0^\pi Y_{lm}^*(\theta,\phi)Y_{l'm'}(\theta,\phi)\sin\theta\,d\theta\,d\phi=\delta_{ll'}\delta_{mm'}$$

> 注：$Y_{lm}$ 是 $\hat{L}^2$ 与 $\hat{L}_z$ 的共同本征函数。
...
其中 $V(r)=-\frac{e^2}{r}$，$k=\frac{\sqrt{-2\mu E}}{\hbar}$，则方程为：
...
$$\beta=\frac{\mu e^2}{\hbar^2 k}=\frac{\mu e^2}{\hbar^2}\cdot\frac{\hbar}{\sqrt{-2\mu E}}=n$$
...
$$L_z=m\hbar,\quad L_z Y_{lm}=m\hbar Y_{lm}$$
...
碱金属原子与氢原子的区别在于：碱金属原子中，价电子感受到的势场不再是纯库仑势，而是被内层电子屏蔽后的势场。其势能可写为：

$$V(r)=-\frac{ke^2}{r}-\frac{2\lambda}{r^2}$$

（氢原子：$V(r)=-\frac{ke^2}{r}$；碱金属原子：$V(r)=-\frac{ke^2}{r}-\frac{2\lambda}{r^2}$），其中 $a_0$ 为玻尔半径。

径向方程为：

$$\left[\frac{d^2}{dr^2}+\frac{2\mu}{\hbar^2}\left(E+\frac{ke^2}{r}+\frac{2\lambda}{r^2}\right)-\frac{l(l+1)}{r^2}\right]R_l(r)=0$$

即：

$$\left[\frac{d^2}{dr^2}+\frac{2\mu}{\hbar^2}\left(E+\frac{ke^2}{r}\right)+\frac{2\mu}{\hbar^2}\frac{2\lambda}{r^2}-\frac{l(l+1)}{r^2}\right]R(r)=0$$
...
矢势 $\mathbf{A}=(A_x,\,A_y,\,A_z)$。

由 $\nabla\times\mathbf{B}$ 的展开：
...
$$F_x=q\left[-\frac{\partial\varphi}{\partial x}-\frac{dA_x}{dt}+\frac{\partial A_x}{\partial x}\dot{x}+\frac{\partial A_x}{\partial y}\dot{y}+\frac{\partial A_x}{\partial z}\dot{z}\right]=q\left(-\frac{\partial\varphi}{\partial x}-\frac{dA_x}{dt}+\mathbf{v}\cdot\nabla A_x\right)$$
...
$$\frac{\partial U}{\partial x}=q\frac{\partial\varphi}{\partial x}-q\frac{\partial}{\partial x}(\mathbf{A}\cdot\mathbf{v})=-q\frac{\partial}{\partial x}(\mathbf{A}\cdot\mathbf{v})+q\frac{\partial\varphi}{\partial x}$$
...
$$L=T-q\varphi+q\mathbf{A}\cdot\mathbf{v}=\frac{1}{2}m\mathbf{v}^2-q\varphi+q\mathbf{A}\cdot\mathbf{v}$$
...
**三维欧氏空间**，线元 $ds^2 = (dx)^2 + (dy)^2 + (dz)^2 = \sum_{ij} g_{ij} dx^i dx^j$，$\delta = \begin{pmatrix} 1 & 0 & 0 \\ 0 & 1 & 0 \\ 0 & 0 & 1 \end{pmatrix}$
...
角向方程：  
\[
\sin\theta \frac{\partial}{\partial\theta}\left(\sin\theta \frac{\partial}{\partial\theta}\right) + \frac{\partial^2}{\partial\phi^2} \quad \text{作用在} \quad Y(\theta,\phi) \quad \text{上，本征值为} \quad -l(l+1)
\]  
其中 \(l\) 为角量子数，\(m\) 为磁量子数，且满足 \( -l \le m \le l \)。
...
设 $P$ 为向量场，考虑平行于 $d$ 轴的四个面，在剩下两面中一个面的通量为：
...
$$\therefore (\nabla \cdot \mathbf{F}) V = (\nabla \cdot \mathbf{F}) J dudvdw = \left( \frac{\partial J}{\partial u} dudvdw + \frac{\partial J}{\partial v} dudvdw + \frac{\partial J}{\partial w} dudvdw \right)$$
...
考虑 $\text{rot } \mathbf{a}$ 在 $u$ 轴上的投影，取 $\mathbf{n}$ 为正方向，$S$ 面是 $u = \text{常数}$，曲面 $S$ 中的曲线 $L$ 设为 $M M_2 N_2 N_1$。

$$\int_{M M_2} \mathbf{a} \cdot d\mathbf{l} = \mathbf{a}(u, v, w) \cdot d\mathbf{l} = \mathbf{a}(u, v, w) \cdot \mathbf{H}_v dv = a_v (u, v, w) H_v (u, v, w) dv$$

点 $N_2$ 坐标为 $(u, v + dv, w + dw)$
...
$$\int_{N_1 M} \mathbf{a} \cdot d\mathbf{l} = -\mathbf{a}(u, v, w) \cdot \mathbf{H}_w dw = -a_w (u, v, w) H_w (u, v, w) dw$$

则

$$\oint \mathbf{a} \cdot d\mathbf{l} = \left[ \frac{\partial (a_w H_w)}{\partial v} - \frac{\partial (a_v H_v)}{\partial w} \right] dv dw = (\text{rot } \mathbf{a})_u \, dv dw = \frac{1}{H_v H_w} \left[ \frac{\partial (a_w H_w)}{\partial v} - \frac{\partial (a_v H_v)}{\partial w} \right] H_u H_v H_w$$
...
考虑rot a在u轴上的投影，取n为正方向，S面是u=常数，曲面S中的曲线L设为$MM_{2}N_{2}N_{1}M$。
...
选 $A=(-y,x,0)$，则 $=\frac{}{2\mu}^2+\frac{P_x^2+P_y^2}{2\mu}+V(r)$

$\frac{}{2\mu}(x-y)+x^2+y^2+V()_1=，_2=x^2+y^2=x-y$

$A_{1m}=E_{4nm}$，$4nm(r,\theta,\phi)=R_1(r)Y_{lm}(\theta,\phi)$

则 $\left[\frac{}{2\mu}\left(r^2+\right)+\right]R(r)Y(\theta,\phi)=-\left(+\right)+\frac{m}{2\mu}R(r)Y$

于是有 $\frac{}{2\mu}\cdot\left(r^2\frac{d}{dr}\right)+\frac{}{2\mu r^2}\frac{d}{dr}\left(r^2\frac{dR}{dr}\right)+\frac{}{2\mu r^2}\frac{d}{dr}\left(r^2\frac{dR}{dr}\right)$

**注意**①左式 $=\frac{}{2\mu}\left[\frac{1}{r^2}\frac{d}{dr}\left(r^2\frac{d}{dr}\right)-\frac{l(l+1)}{r^2}\right]+\frac{}{2\mu r^2}\frac{d}{dr}\left(r^2\frac{dR}{dr}\right)=E R(\theta,\phi)$

$-\frac{}{2\mu}\left(r^2\frac{dR}{dr}\right)Y(\theta,\phi)-\frac{}{2\mu r^2}\frac{d}{dr}\left(r^2\frac{dR}{dr}\right)+\frac{}{2\mu r^2}\frac{d}{dr}\left(r^2\frac{dR}{dr}\right)=E R(\theta,\phi)$

$\frac{}{2\mu}\left(r^2\frac{dR}{dr}\right)+\frac{}{2\mu r^2}\frac{d}{dr}\left(r^2\frac{dR}{dr}\right)-E=\frac{l(l+1)}{2\mu r^2}+\frac{}{2\mu r^2}$

令 $E_0=E-\omega_1 m\hbar=E-m$，代 $\lambda V(r)=-k-\lambda k$

则 $\frac{}{2\mu}\left(r^2\frac{dR}{dr}\right)-\frac{}{2\mu r^2}\left(-\frac{k^2}{r^2}-\lambda k\right)-\left(\omega_1 m-E\right)=0$

$\frac{d^2R}{dr^2}+\frac{2}{r}\frac{dR}{dr}+\left(E_0+\frac{}{r}\right)R(r)=0$，对比碱金属原子方程，有 $E_0=-\frac{}{n^2}=-\frac{}{}$
...
$$P = \frac{2mv_x}{\Delta t} \times \frac{N}{bc} = \frac{ab}{V}$$

$$v^2 = v_x^2 + v_y^2 + v_z^2 = \frac{3}{2} \bar{v}^2$$

$$E_k = \frac{1}{2} m \bar{v}^2$$ 为粒子的平均动能。

$$\therefore P = \frac{2}{3} n E_k = nkT \Rightarrow E_k = \frac{3}{2} kT$$

## 分子的平均动能与理想气体系统的内能

只考虑分子平动动能，内能 $U = E_k = N \cdot \frac{3}{2} kT = \frac{3}{2} RT$
...
系统转动惯量 $I = m_1 r_1^2 + m_2 r_2^2 = \mu r^2$，系统转动能量

$$T = \frac{1}{2}m_1 r_1^2(\dot{\theta}^2 + \sin^2\theta\,\dot{\varphi}^2) + \frac{1}{2}m_2 r_2^2(\dot{\theta}^2 + \sin^2\theta\,\dot{\varphi}^2)$$

即

$$T = \frac{1}{2}\mu r^2(\dot{\theta}^2 + \sin^2\theta\,\dot{\varphi}^2) = \frac{1}{2\mu r^2}\left(P_\theta^2 + \frac{P_\varphi^2}{\sin^2\theta}\right)$$
...
粒子的状态是连续的，粒子和系统的微观运动状态不可数，人为划出最小的格子。将 $p,q$ 分为等间隔区域，$\delta p_i \delta q_i = h_0$（$\delta q_1 \delta p_1 \cdots \delta q_r \delta p_r = h_0^r$），相格 $h_0^r$ 代表一个粒子态。子相空间小体元 $\Delta \mu_j$（$j=1,2,\cdots,l$）中粒子运动状态数为 $\Delta \omega_j = \dfrac{\Delta \mu_j}{h_0^r}$，$a_j$ 个粒子在 $\Delta \omega_j$ 个运动状态上分布的可能微观状态数为 $\dfrac{(\Delta \omega_j)^{a_j}}{a_j!}$，多个粒子可以处于同一相格内。
...
$S=\pi\sqrt{2mE}$，$\Delta S=\Delta$，$\Delta E=h\nu$ 时，$\Delta S=h$。
...
$$\Phi(E)=\frac{1}{h^3}\cdot\frac{\pi}{8}\cdot\left(2mE\right)^{3/2}\cdot abc=\frac{V}{h^3}\cdot\frac{4\pi}{3}(2mE)^{3/2}$$

$$p_r^2=2\mu E_r,\quad p_\theta^2=\frac{L^2}{r^2\sin^2\theta},\quad p_\varphi^2=\frac{L_z^2}{r^2\sin^2\theta}$$
...
- $A|\psi\rangle$：在右矢空间中，$|\psi\rangle \to |A\psi\rangle$，默认 $A$ 向右作用；在左矢空间中，$\langle \psi| \to \langle \psi|A^\dagger$，算符 $A^\dagger$（$A$ 的厄米共轭）向左作用。$\langle \psi|A^\dagger = \langle A\psi| = \langle \psi|A$。若 $A$ 为厄米算符，则 $\langle \psi|A = \langle A\psi|$。
...
- $(AB|\psi\rangle)^\dagger = \langle \psi|B^\dagger A^\dagger$（先将 $B|\psi\rangle$ 视为一体，再作变换）。
...
$$[l_y, p_z] = -i\hbar p_x, \quad [l_y, p_x] = i\hbar p_z, \quad [l_z, p_x] = i\hbar p_y, \quad [l_z, p_y] = -i\hbar p_x$$

### 角动量分量之间的对易式

$$[l_x, l_x] = 0, \quad [l_\alpha, l_\alpha] = 0 \quad (\alpha = x, y, z)$$

$$[l_x, l_y] = [yp_z - zp_y, zp_x - xp_z] = [y, z]p_x p_z - [z, xp_z] + [z, xp_z]$$

$$= [y, zp_x] + [z, xp_y] = y[p_z, l_y] + x[-l_z, p_y] = -y(i\hbar p_x) - x(-i\hbar p_y) = i\hbar l_z$$

**同理：** $[l_y, l_z] = i\hbar l_x$，$[l_z, l_x] = i\hbar l_y$

### 角动量平方算符

令 $l^2 = l_x^2 + l_y^2 + l_z^2 = y^2 p_z^2 + z^2 p_y^2 - y p_z (z p_y) - z p_y (y p_z)$

则 $l^2 = (x^2 + y^2)p_z^2 + (z^2 + x^2)p_y^2 + (y^2 + z^2)p_x^2 - (yz p_z p_y - p_z) - (yz p_y p_z - p_z) - (zx p_z p_x - p_x)$

$$- (zx p_x p_z - p_x) - (xy p_x p_y - i\hbar p_x) - (yx p_y p_x - y p_x)$$
...
同理，对于反对易关系 $\{A, BC\}$ 也有类似的展开形式，且当 $A$ 与 $B$ 对易时，相应项为零。
...
$$\langle\lambda\hat{A}+i\hat{B}|\psi\rangle^\dagger(\lambda\hat{A}+i\hat{B})|\psi\rangle = (\lambda\langle\psi|\hat{A} - i\langle\psi|\hat{B})(\lambda\hat{A}|\psi\rangle + i\hat{B}|\psi\rangle)$$
...
**对任意力学量 $A$、$B$，对任意量子态 $|\psi\rangle$，若 $A$ 与 $B$ 不对易（即 $[\hat{A},\hat{B}]\neq 0$），则 $\Delta A$ 和 $\Delta B$ 不能同时为零，也即 $A$ 与 $B$ 不能同时测定。**
...
证明：
$$\langle \psi | A | \psi \rangle = \langle \psi | A^\dagger | \psi \rangle = \langle \psi | A | \psi \rangle^* = \langle \psi | A | \psi \rangle$$
∴ $A = A^\dagger$，$A$ 为实数。

或：
$$\langle \psi | A^\dagger | \psi \rangle = \langle A\psi | \psi \rangle, \quad \langle \psi | A | \psi \rangle = \langle A^\dagger \psi | \psi \rangle$$
$A = A^\dagger = A^*$，$A$ 为实数。

$$\langle \psi | A^2 | \psi \rangle = \langle \psi | AA | \psi \rangle = \langle \psi | A^\dagger A | \psi \rangle = \langle A\psi | A\psi \rangle = \langle \Phi | \Phi \rangle \ge 0$$
∴ $A^2 \ge 0$，其中 $|\Phi\rangle = A|\psi\rangle$。
...
**定理**：哈密顿算符 $H$ 为厄米算符，满足本征方程 $H|k\rangle = E_k |k\rangle$。对体系的任一归一化态 $|\psi\rangle$，若 $\langle \psi | H | \psi \rangle$ 有下界（总大于某常数）但无上界，则 $H$ 的本征态 $|k\rangle$ 的集合构成体系的一个**完备集**，即体系的任一量子态 $|\psi\rangle$ 可用 $|k\rangle$ 来展开。
...
$\hat{H}$ 有共同本征函数 $\psi$，$\hat{F}\psi=F\psi$，$\hat{H}\psi=E\psi$。∵ $[\hat{G},\hat{H}]=0$，则 $\hat{G}\hat{H}\psi=\hat{H}(\hat{G}\psi)=E(\hat{G}\psi)$，即 $\hat{G}\psi$ 也是 $\hat{H}$ 的本征态，本征值仍为 $E$。
...
从矢量的角度考虑，$|\psi\rangle$ 是列向量 $e_k$ 的线性组合。要消去基矢，则基矢组之间通过 $S$ 矩阵相联系。

由正交归一性：$\langle k'|k\rangle = \delta_{k'k}$，$\langle k|k\rangle = 1$，$|k\rangle = \sum_\beta S_{\beta k}|\beta\rangle$，则 $\langle k'|k\rangle = \left(\sum_\beta S^*_{\beta k'}\langle \beta|\right)\left(\sum_\gamma S_{\gamma k}|\gamma\rangle\right) = \sum_\beta S^*_{\beta k'} S_{\beta k} = \delta_{k'k}$。
...
基矢变换关系：$(e'_1, e'_2, \cdots, e'_k, \cdots) = (e_1, e_2, \cdots, e_k, \cdots) S$，即新基矢是旧基矢通过 $S$ 矩阵的线性组合。
