...
3. **$x>a$ 区域**：方程化为 $\psi''(x) - \frac{2\mu}{\hbar^2}(V_0 - E)\psi(x) = 0$。令 $\beta = \sqrt{\frac{2\mu(V_0 - E)}{\hbar^2}}$，则 $\psi(x) = Be^{\beta x} + Ce^{-\beta x}$。
...
- 在 $x=a$ 处连续：$\psi(a) = Be^{-\beta a} = A\sin(ka)$；导数连续：$\psi'(a) = -\beta Be^{-\beta a} = kA\cos(ka)$。
...
令 $\alpha = \sqrt{\frac{\mu \omega}{\hbar}}$，$\xi = \alpha x$，记 $\psi(x) = \phi(\xi) = \phi(\alpha x)$，于是 $\left[ -\frac{d^2}{d\xi^2} + \xi^2 \right] \phi(\xi) = \lambda \phi(\xi)$

即 $-\frac{d^2 \phi(\xi)}{d\xi^2} + \xi^2 \phi(\xi) = \lambda \phi(\xi)$，整理得 $\frac{d^2 \phi}{d\xi^2} + (\lambda - \xi^2) \phi(\xi) = 0$

为“消除”$\xi^2$ 项，试探设 $\phi(\xi) = e^{-\xi^2/2} H(\xi)$，代入得 $H''(\xi) - 2\xi H'(\xi) + (\lambda - 1) H(\xi) = 0$

$\phi(\xi)$ 有界（平方可积）。仅当 $\lambda = 2n + 1$ 时，$H_n(\xi)$ 有（多项式）解，$H(\xi) = H_n(\xi) = (-1)^n e^{\xi^2} \frac{d^n}{d\xi^n} e^{-\xi^2}$

正交归一性：$\int_{-\infty}^{\infty} H_m(\xi) H_n(\xi) e^{-\xi^2} d\xi = \sqrt{\pi} \, 2^n n! \, \delta_{mn}$

归一化波函数：$\phi_n(\xi) = N_n e^{-\xi^2/2} H_n(\xi)$，$N_n^2 \int e^{-\xi^2} H_n(\xi) H_n(\xi) d\xi = 1$，$N_n = \left( \frac{\alpha}{\sqrt{\pi} \, 2^n n!} \right)^{1/2}$
...
角向方程：  
\[
\sin\theta \frac{\partial}{\partial\theta}\left(\sin\theta \frac{\partial}{\partial\theta}\right) + \frac{\partial^2}{\partial\phi^2} + \ell(\ell+1)\sin^2\theta = 0
\]

设 $H(\xi) = \sum_j a_j \xi^j$，代入得递推关系：$a_{j+2} (j+2)(j+1) - 2j a_j + (\lambda - 1) a_j = 0$，即 $a_{j+2} = \frac{2j - (\lambda - 1)}{(j+2)(j+1)} a_j$

$\phi(\xi) \sim e^{\xi^2}$（按 $e^{\xi^2}$ 量级增长），不可积。级数存在（只有）有限项：$a_n \neq 0$，$a_{n+2} = 0$，即 $\lambda = 2n + 1$。

从而有 $a_{j-2} = \frac{2j - 2n}{(j)(j-1)} a_j$，多项式 $H(\xi)$ 只能含奇数项或偶数项，系数由高次项推至低次项。

方程可变化为 $H''(\xi) - 2\xi H'(\xi) + 2n H(\xi) = 0$，其解为厄米多项式 $H_n(\xi)$。
...
母函数 $w(t, x) = e^{2xt - t^2}$，满足 $\frac{\partial w(t, x)}{\partial t} + 2(t - x) w(t, x) = 0$
...
> **注意：** 球谐函数 $Y_{lm}(\theta,\phi)$ 是角动量平方算符 $\hat{L}^2$ 和角动量 $z$ 分量 $\hat{L}_z$ 的共同本征函数。
...
$j_\varphi = \frac{e\hbar m}{m_e r \sin\theta} |\psi|^2 = \frac{e\hbar m}{m_e r \sin\theta} |R_{nl} P_l^m|^2$。
...
$$\left[\frac{d^2}{dr^2}+\frac{2\mu}{\hbar^2}\left(E+\frac{e^2}{r}\right)+\frac{2\mu ke^2/\hbar^2-l(l+1)}{r^2}\right]R(r)=0$$
...
$$\frac{d}{dt}\left(\frac{\partial L}{\partial \dot{x}}\right)-\frac{\partial L}{\partial x}=0, \quad y、z \text{ 同理}$$
...
又 $\left(\frac{\partial u}{\partial x}, \frac{\partial u}{\partial y}, \frac{\partial u}{\partial z}\right) = \left(\frac{\partial u}{\partial q_1}, \frac{\partial u}{\partial q_2}, \frac{\partial u}{\partial q_3}\right) \frac{\partial(q_1, q_2, q_3)}{\partial(x, y, z)}$

即

$$\frac{\partial(u, v, w)}{\partial(x, y, z)} = \frac{\partial(u, v, w)}{\partial(q_1, q_2, q_3)} \times \frac{\partial(q_1, q_2, q_3)}{\partial(x, y, z)}$$

$$\frac{\partial(u, v, w)}{\partial(x, y, z)} = \frac{\partial(u, v, w)}{\partial(q_1, q_2, q_3)} \cdot \frac{\partial(q_1, q_2, q_3)}{\partial(x, y, z)}$$

$$\frac{\partial(x, y, z)}{\partial(u, v, w)} = \frac{\partial(x, y, z)}{\partial(q_1, q_2, q_3)} \cdot \frac{\partial(q_1, q_2, q_3)}{\partial(u, v, w)}$$
...
$$P = 2mv_x \times \frac{N}{V} = \frac{1}{3} nm\overline{v^2}$$

$$v^2 = \frac{3kT}{m}, \quad \frac{1}{2}m\overline{v^2} = \frac{3}{2}kT$$

$E_t = \frac{1}{2}m\overline{v^2}$ 为粒子的平均动能。

$$\therefore P = \frac{2}{3}nE_t = nkT \Rightarrow E_t = \frac{3}{2}kT$$

## 分子的平均动能与理想气体系统的内能

只考虑分子平动动能，$U = E = N E_t = \frac{3}{2}NkT = \frac{3}{2}RT$
...
质量为m的粒子在弹性力F=-Ax作用下在平衡点附近作简谐运动，振动圆频率$w=\sqrt{A/m}$。  
以x、P为直角坐标可构成二维相空间，若ε给定，则代表点的轨迹是椭圆，椭圆面积=π$\sqrt{2mε}$。  
③转子(双原子分子整体刚性转动)
...
一维线性谐振子 $E=\frac{p^2}{2m}+\frac{1}{2}m\omega^2 x^2$，$S=\pi\sqrt{\frac{2E}{m}}$，代表点的轨迹为椭圆（μ空间）。
...
对于一个自由度为 $r$ 的粒子，它的 μ 空间中大小为 $h^r$ 的相体积对应一个量子态。第一象限 $0 \sim \varepsilon$ 范围内的总量子态数

$$\Omega(\varepsilon) = \frac{1}{h^3} \cdot \frac{4\pi}{3} \cdot (2m\varepsilon)^{3/2} \cdot V = \frac{4\pi V}{3h^3}(2m\varepsilon)^{3/2}$$

其中 $V = abc$，$p^2 = 2m\varepsilon$，$p_r^2 = p_x^2 + p_y^2 + p_z^2$，$p_r^2 \sin^2\theta$ 等。

（注：第 19、20 行 OCR 内容无法辨识，已省略。）
...
- 关于 $(|A4\rangle)^\dagger = \langle 4|A^\dagger$：$\bar{A} = \langle 4|A|4\rangle$，$\bar{A}^* = \langle 4|A^\dagger|4\rangle$，$\langle 4|A|4\rangle^* = \langle 4|A^\dagger|4\rangle$，$\langle A4| = \langle 4|A^\dagger$。
...
- $[l_x, l_y] = (y p_z - z p_y)(z p_x - x p_z) - (z p_x - x p_z)(y p_z - z p_y)$  
  展开后合并同类项，可得 $[l_x, l_y] = i\hbar l_z$（其余循环类推）。
...
- $l_y l_z l_x - l_z = (l - l_y) + l l_x - l_x = [l_y, l_2] + 2l_y - y_2$  
  方法将本征态对应的算符尽量转化到最左/右边，直接作用到态矢上。
...
$=\frac{m}{i}[\langle lm|l_y|lm\rangle-\langle lm|l_y|lm\rangle]=0$
...
$l_y l_x-l_x l_y=(l_+l_-+l_-l_+)-(l_+l_-+l_-l_+)=0$
...
其中 $[\hat{A},\hat{B}]=\hat{A}\hat{B}-\hat{B}\hat{A}$ 对易子，$\Delta A=\sqrt{\langle(\hat{A}-\langle A\rangle)^2\rangle}$，$\Delta B=\sqrt{\langle(\hat{B}-\langle B\rangle)^2\rangle}$。

证明：

令 $x=\hat{A}-\langle A\rangle$，$y=\hat{B}-\langle B\rangle$，要证变为 $\langle x^2\rangle\langle y^2\rangle \ge |\langle[x,y]\rangle|^2$，联想 $b^2 \le 4ac$。
...
$$= \lambda^2\langle A^2\rangle + \langle B^2\rangle + i\lambda\langle[\hat{A},\hat{B}]\rangle \ge 0$$

对任意 $\lambda$ 成立，故判别式 $\le 0$，即：

$$\langle[\hat{A},\hat{B}]\rangle^2 \le 4\langle A^2\rangle\langle B^2\rangle$$

即 $\langle x^2\rangle\langle y^2\rangle \ge \frac{1}{4}|\langle[\hat{A},\hat{B}]\rangle|^2$，因此：
...
证明：$[\hat{A},\hat{B}]=0 \Rightarrow \hat{A}\hat{B}=\hat{B}\hat{A}$，则 $\hat{B}\hat{A}|k\rangle = \hat{B}\cdot a_k|k\rangle = a_k\hat{B}|k\rangle$。因此 $\hat{A}(\hat{B}|k\rangle) = a_k(\hat{B}|k\rangle)$，所以 $\hat{B}|k\rangle$ 也是 $A$ 属于本征值 $a_k$ 的本征态。由于 $a_k$ 不简并，$\hat{B}|k\rangle$ 与 $|k\rangle$ 是同一个量子态，即 $\hat{B}|k\rangle = b_k|k\rangle$。故 $A$、$B$ 拥有共同本征态 $|k\rangle$。
...
$\therefore \lambda_k = F$ 为实数（$\langle k|k\rangle = 1$）。
