...
$x>a$区域，有$\psi''(x)-\frac{2\mu}{\hbar^2}(V_0-E)\psi(x)=0$。令$\beta=\sqrt{\frac{2\mu(V_0-E)}{\hbar^2}}$，则$\psi(x)=Be^{\beta x}+Ce^{-\beta x}$
...
且$\cot ka<0$。$E$满足超越方程$E=\frac{\hbar^2 k^2}{2\mu}$，$\sin^2 ka=\frac{\hbar^2}{2\mu a^2\beta}\sin^2 ka$，且$\cot(ka)<0$，能量是量子化的。
...
**径向方程**：$\left[\frac{1}{r^2}\frac{d}{dr}\left(r^2\frac{d}{dr}\right)\right]R(r) + \frac{2\mu}{\hbar^2}(E-V(r))R(r)=0$

**角向方程**：$\frac{1}{\sin\theta}\frac{\partial}{\partial\theta}\left(\sin\theta\frac{\partial}{\partial\theta}\right)Y(\theta,\phi) + \frac{1}{\sin^2\theta}\frac{\partial^2}{\partial\phi^2}Y(\theta,\phi) + \lambda Y(\theta,\phi)=0$
...
$|m|\le l$ 时才有 $\Theta(\theta)\neq 0 \Rightarrow m=0,\pm1,\cdots,\pm l$

$\Theta(\theta)\Phi(\phi)\sin\theta d\theta d\phi = \delta_{ll'}\delta_{mm'}$，$N_{lm} = \sqrt{\frac{(2l+1)(l-|m|)!}{4\pi(l+|m|)!}}$，$Y_{lm}(\theta,\phi)=N_{lm}P_l^m(\cos\theta)e^{im\phi}$

**球谐函数满足正交关系**：$\int_0^{2\pi}\int_0^\pi Y_{lm}^*(\theta,\phi)Y_{l'm'}(\theta,\phi)\sin\theta d\theta d\phi = \delta_{ll'}\delta_{mm'}$
...
**注意：** $\because V(r)<0$，$R_l(r)\to 0$，$r\to\infty$ 时薛定谔方程约为 $\frac{d^2u}{dr^2}+\frac{2\mu E}{\hbar^2}u=0$，若 $E>0$，$u(r)$ 呈振荡形式，**不满足束缚态条件**，则 $E<0$。从能量角度分析，$E=V+K$，$K<V$，$E<0$。核与电子构成“双星模型”。
...
令 $n=l+1+n_r$，$n=1,2,\dots$，则 $\beta=n$，$\therefore E_n=-\frac{\mu e^4}{2\hbar^2 n^2}$。
...
**径向位置概率分布：** 在 $(r, r+dr)$ 内概率为 $r^2 dr \int |\psi_{nlm}(r,\theta,\phi)|^2 \sin\theta\, d\theta\, d\phi = r^2 |R_{nl}(r)|^2 dr = |u_{nl}(r)|^2 dr$。
...
$$\frac{d}{dt}\left(\frac{\partial T}{\partial \dot{x}}\right)-\frac{\partial T}{\partial x}-\frac{\partial}{\partial x}(-q\phi+q\mathbf{A}\cdot\mathbf{v})=0$$
...
对标量函数 $u(q_1, q_2, q_3)$，在增长方向的梯度 $\nabla u = \sum_i \frac{1}{h_i} \frac{\partial u}{\partial q_i} \hat{e}_i$
...
$$= \frac{\partial (u, v, w)}{\partial (x, y, z)} \times \cdots$$
...
$$\therefore (\nabla \cdot \mathbf{F}) V = (\nabla \cdot \mathbf{F}) J \, du \, dv \, dw = \left( \frac{\partial}{\partial u} + \frac{\partial}{\partial v} + \frac{\partial}{\partial w} \right) J \, du \, dv \, dw$$
...
选 $A=(-y，x，0)$，则 $H=\frac{1}{2\mu}(p_x^2+p_y^2)+V(r)$

$\frac{1}{2\mu}(p_x^2+p_y^2)+V(r)$，$\rho^2=x^2+y^2$

$A_{lm}=E_{nl} \psi_{nlm}(r,\theta,\phi)=R_{nl}(r)Y_{lm}(\theta,\phi)$

则 $\frac{1}{r^2}\frac{d}{dr}(r^2\frac{dR}{dr})+[\frac{2\mu}{\hbar^2}(E-V)-\frac{l(l+1)}{r^2}]R=0$

于是有 $\frac{1}{r^2}\frac{d}{dr}(r^2\frac{dR}{dr})+$

**注意**①左式 $=\frac{1}{r^2}\frac{d}{dr}(r^2\frac{dR}{dr})+[\frac{2\mu}{\hbar^2}(E-V)-\frac{l(l+1)}{r^2}]R=0$

$-\frac{1}{r^2}\frac{d}{dr}(r^2\frac{dR}{dr})+[\frac{l(l+1)}{r^2}-\frac{2\mu}{\hbar^2}(E-V)]R=0$
...
则 $\frac{1}{r^2}\frac{d}{dr}(r^2\frac{dR}{dr})-[\frac{2\mu}{\hbar^2}(-k-\lambda k)-(\omega_1 m-E)]R=0$

$\frac{1}{r^2}\frac{d}{dr}(r^2\frac{dR}{dr})+[\frac{2\mu}{\hbar^2}(E_0+\frac{\lambda}{r})]R(r)=0$，对比碱金属原子方程，有 $E_0=-\frac{1}{n^2}$
...
质量为m的粒子在弹性力F=-Ax作用下在平衡点附近作简谐运动，振动圆频率$w=\sqrt{\frac{A}{m}}$。其能量表达式为
...
以x、P为直角坐标可构成二维相空间，若ε给定，则代表点的轨迹是椭圆，椭圆面积=π$\sqrt{\frac{2m\varepsilon}{m\omega^2}}\cdot\sqrt{2m\varepsilon}$，即等于$2\pi\varepsilon/\omega$。
...
设两质点质量分别为 $m_1, m_2$，距质心距离分别为 $r_1, r_2$，约化质量 $\mu = \frac{m_1 m_2}{m_1 + m_2}$，系统转动能量：

$$T = \frac{1}{2}m_1 r_1^2(\dot{\theta}^2 + \sin^2\theta\,\dot{\varphi}^2) + \frac{1}{2}m_2 r_2^2(\dot{\theta}^2 + \sin^2\theta\,\dot{\varphi}^2) = \frac{1}{2}\mu r^2(\dot{\theta}^2 + \sin^2\theta\,\dot{\varphi}^2)$$
...
粒子的状态是连续的，粒子和系统的微观运动状态不可数，人为划出最小的格子。将 $p, q$ 分为等间隔区域，$\delta p_j \delta q_j = h_0$（$\delta q_1, p_1$）…（$\delta q_r, p_r$）= $h_0$，相格 $h_0$ 代表一个粒子态。子相宇空间小体元 $\Delta \mu_j$（$j=1,2,\cdots,l$）中粒子运动状态数为 $\Delta \omega_j = \dfrac{\Delta \mu_j}{h_0}$，$a_j$ 个粒子在 $\Delta \omega_j$ 个运动状态上分布的可能微观状态数为 $\begin{pmatrix} \Delta \omega_j + a_j - 1 \\ a_j \end{pmatrix}$，多个粒子可以处于同一相格内。
...
则 $\delta(\ln a_j! - \ln \mu_j!) \delta a_j = (\ln a_j + \ln \mu_j - \ln \mu_j) \delta a_j = (\ln a_j - \ln \mu_j) \delta a_j = 0$
...
$\sum a_j = N$，$\therefore \sum \mu_j e^{-\alpha - \beta \varepsilon_j} = N$
...
$\Delta x\Delta P_x \ge \frac{h}{2}$，一个粒子的代表不再是点，而是一团“小空间”，粒子的状态是分立的，不再需要用“μ空间”，只需考虑分立能级上的分布即可。
...
第一象限 $0\sim$ 范围内的总量子态数 $\Omega(E)=\frac{1}{h^3}\times\frac{4}{3}\pi(2mE)^{3/2}abc=\frac{4\pi V}{3h^3}(2mE)^{3/2}$。

$2\mu E r_0 \sin^2\theta$，$P_\theta^2$。
...
- 设 $A$ 和 $\tilde{A}$ 是两个算符，对 $|4\rangle$ 和 $|4\rangle$，若 $\langle \varphi|A|4\rangle = \langle \varphi|\tilde{A}|4\rangle$，则称 $\tilde{A}$ 为 $A$ 的转置算符。
- $A|4\rangle = |A4\rangle$：在右矢空间中，$|4\rangle \to |4'\rangle = |A4\rangle$，默认 $A$ 向右作用；在左矢空间中，$\langle 4| \to \langle 4'| = \langle 4|A^\dagger$，算符 $A^\dagger$（$A$ 的厄米共轭）向左作用。$\langle 4|A^\dagger = \langle A4| = \langle 4'|$。若 $A$ 为厄米算符，则 $\langle 4'| = \langle 4|A$。
...
$l_y l_z l_x-l_z^2=(l_+l_-)+l_z l_x-l_x l_z=[l_y,l_z]+2l_y l_z-l_y l_z$

$l^2=l_x^2+l_y^2+l_z^2=(l_+l_-+l_-l_+)+l_z^2=l(l+1)\hbar^2+m\hbar(l_z-l_z)=l(l+1)\hbar^2+m\hbar(l_z-l_z)=$

设 $l_x=l_y=\frac{1}{2}(l_+^2+l_-^2)=[l(l+1)-m^2]\hbar^2$
...
其中 $[\hat{A},\hat{B}]=\hat{A}\hat{B}-\hat{B}\hat{A}$ 成立，$\Delta A=\overline{A-A}$，$\Delta B=\overline{B-B}$，也即：

$$[\hat{A},\hat{B}]=[\Delta\hat{A}+\bar{A},\Delta\hat{B}+\bar{B}]=(\Delta\hat{A}+\bar{A})(\Delta\hat{B}+\bar{B})-(\Delta\hat{B}+\bar{B})(\Delta\hat{A}+\bar{A})=[\Delta\hat{A},\Delta\hat{B}]$$

$$\Delta A=\sqrt{\overline{\hat{A}^2}-\bar{A}^2}=\sqrt{\overline{(\hat{A}-\bar{A})^2}},\quad \Delta B=\sqrt{\overline{\hat{B}^2}-\bar{B}^2}$$

要证：

$$\overline{(\hat{A}-\bar{A})^2}\cdot\overline{(\hat{B}-\bar{B})^2}\ge \frac{1}{4}|\langle[\hat{A}-\bar{A},\hat{B}-\bar{B}]\rangle|^2$$

令 $x=\hat{A}-\bar{A}$，$y=\hat{B}-\bar{B}$，要证变为：

$$\overline{x^2}\cdot\overline{y^2}\ge \frac{1}{4}|\langle[x,y]\rangle|^2$$

联想 $b^2\le 4ac$。

考虑 $|\phi\rangle=(\xi\hat{A}+i\eta\hat{B})|\psi\rangle$，$\langle\phi|\phi\rangle\ge 0$：

$$\langle\psi|(\xi\hat{A}+i\eta\hat{B})^\dagger(\xi\hat{A}+i\eta\hat{B})|\psi\rangle=(\xi\langle\psi|\hat{A}-i\eta\langle\psi|\hat{B})(\xi\hat{A}|\psi\rangle+i\eta\hat{B}|\psi\rangle)$$

$\hat{A},\hat{B}$ 为厄米算符：

$$=(\xi\langle\psi|\hat{A}-i\eta\langle\psi|\hat{B})(\xi\hat{A}|\psi\rangle+i\eta\hat{B}|\psi\rangle)$$

$$=\xi^2\langle\psi|\hat{A}^2|\psi\rangle+\eta^2\langle\psi|\hat{B}^2|\psi\rangle+i\xi\eta(\langle\psi|\hat{A}\hat{B}|\psi\rangle-\langle\psi|\hat{B}\hat{A}|\psi\rangle)=\xi^2\langle\hat{A}^2\rangle+\eta^2\langle\hat{B}^2\rangle+i\xi\eta\langle[\hat{A},\hat{B}]\rangle$$

$\because \forall\xi,\ \langle\phi|\phi\rangle\ge 0$，$\therefore (\langle i[\hat{A},\hat{B}]\rangle)^2\le 4\langle\hat{A}^2\rangle\cdot\langle\hat{B}^2\rangle$，即：

$$\langle\hat{A}^2\rangle\cdot\langle\hat{B}^2\rangle\ge \frac{1}{4}|\langle[\hat{A},\hat{B}]\rangle|^2$$
...
若对 $\forall |\psi\rangle$，有 $P|\psi\rangle = \sum_k |k\rangle\langle k|\psi\rangle = |\psi\rangle$，则称基矢 $|k\rangle$ 具有**完备性**（任意 $|\psi\rangle$ 可按 $|k\rangle$ 展开）。
...
**完备性与封闭性**：强调重点不同，完备性指任意 $|\psi\rangle$ 可按 $\{|k\rangle\}$ 展开，封闭性指数学上 $\sum_k |k\rangle\langle k| = 1$ 成立，两者相互依存。
...
又 $\hat{F}(\hat{G}\psi) \neq \hat{G}(\hat{F}\psi) = FG\psi$，则 $\hat{G}\psi$ 不是 $\hat{F}$ 的本征态，$\therefore \hat{G}\psi$ 和 $\psi$ 不是一个态，即 $E$ 对应至少两个态，能级简并。
...
则 $a_\beta' = a_k \langle \beta|k\rangle$，即 $a' = S a$。记 $S_{\beta k} = \langle \beta|k\rangle$，$S$ 为变换矩阵。
...
要消去基矢，则基矢组之间的变换为 $e' = e S$，系数列相应变换。

由正交归一性：

- $\langle k'|k\rangle = \delta_{k'k}$，$\langle k|k\rangle = 1$
- $|k\rangle = S_{\beta k} |\beta\rangle$，则 $\langle k'|k\rangle = (S_{\beta k'}^* \langle \beta|)(S_{\beta k}|\beta\rangle) = S_{\beta k'}^* S_{\beta k} = \delta_{k'k}$
...
$$(e_1', e_2', \cdots) = (e_1, e_2, \cdots) S$$

即

$$\begin{pmatrix} e_1' \\ e_2' \\ \vdots \end{pmatrix} = S \begin{pmatrix} e_1 \\ e_2 \\ \vdots \end{pmatrix}$$
